...
│   ├── core/                     # 이벤트 시스템 코어
│   │   ├── __init__.py
│   │   ├── base_event.py         # BaseEvent ABC
│   │   ├── event_producer.py     # EventProducer<T>
│   │   ├── event_consumer.py     # EventConsumer<T>
│   │   └── event_tunnel_manager.py # EventTunnelManager
│   │
│   ├── types/                    # 구체적인 이벤트 타입들
│   │   ├── __init__.py
...
class ExperienceConsumer(EventConsumer[EnemyDeathEvent]):
    """경험치 처리 전용 컨슈머"""
    
    def __init__(self, experience_manager: 'IExperienceManager') -> None:
        """경험치 관리자 의존성 주입"""
        super().__init__(EnemyDeathEvent)
        self._experience_manager = experience_manager
    
    def consume(self, event: EnemyDeathEvent) -> None:
        """적 사망 이벤트 처리 - 경험치 지급"""
