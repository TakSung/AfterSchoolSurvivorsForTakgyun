...
class EventTunnelManager:
    ...
    def register_consumer_system(self, consumer_system: 'System', event_type: type[T]) -> None:
        """Consumer System에 EventConsumer 주입"""
        # AI-NOTE : 2025-08-17 2계층 Consumer 연결 관리
        # - 이유: EventTunnelManager가 기본 Consumer와 System Consumer를 연결
        # - 요구사항: Consumer System이 올바른 EventConsumer를 참조하도록 주입
        # - 구조: EventTunnelManager → EventConsumer → ConsumerSystem
        
        # 1. 터널에서 기본 EventConsumer 가져오기
        _, basic_consumer = self.get_producer(event_type), self.get_consumer(event_type)
        
        # 2. Consumer System에 기본 EventConsumer 주입
        if hasattr(consumer_system, '_event_consumer'):
            consumer_system._event_consumer = basic_consumer
        
        # 3. 등록 로깅
        print(f"Registered {consumer_system.__class__.__name__} for {event_type.__name__}")
...
**2계층 Consumer 구조 사용 예시**:
```python
# EventTunnelManager 사용 예시 - 2계층 Consumer 구조
tunnel_manager = EventTunnelManager()
experience_manager = ExperienceManager()

# 1. Consumer System 생성 (고수준 비즈니스 로직)
experience_consumer_system = ExperienceConsumerSystem(experience_manager)

# 2. EventTunnelManager에 Consumer System 등록
# - 내부적으로 기본 EventConsumer가 Consumer System에 주입됨
tunnel_manager.register_consumer_system(experience_consumer_system, EnemyDeathEvent)

# 3. Producer 가져오기 (시스템에서 이벤트 발행용)
producer = tunnel_manager.get_producer(EnemyDeathEvent)

# 4. Producer가 이벤트 생산 (예: ProjectileSystem에서)
...
# 5. Consumer System이 자동으로 처리 (System.update() 호출 시)
experience_consumer_system.update(delta_time=0.016)
# → 내부 EventConsumer가 SharedEventQueue에서 이벤트 소비
# → ExperienceManager.grant_experience() 호출

# 6. 구조 확인
print("=== 2계층 Consumer 구조 ===")
print(f"Level 1 (기본): {type(experience_consumer_system._event_consumer).__name__}")
print(f"Level 2 (구현): {type(experience_consumer_system).__name__}")
...
├── 기본 EventConsumer (Level 1)       ← 저수준 큐 처리
└── ExperienceConsumerSystem (Level 2) ← 고수준 비즈니스 로직
    └── HAS-A: EventConsumer           ← 컴포지션으로 기본 Consumer 포함
```
...
#### **Event Consumer 정의 (System 기반) - 2계층 Consumer 구조**

**레벨 1: 기본 EventConsumer** (저수준 큐 처리)
```python
# events/core/event_consumer.py - 이미 정의됨
class EventConsumer(Generic[T]):
    """타입 안전한 이벤트 소비자 - 공유 큐 참조"""
    def consume_next(self) -> T | None: ...
    def is_empty(self) -> bool: ...
...
**레벨 2: 구현된 Consumer System** (고수준 비즈니스 로직)
```python
...
class ExperienceConsumerSystem(System):
    ...
    def __init__(self, experience_manager: 'IExperienceManager') -> None:
        ...
        # AI-NOTE : 2025-08-17 2계층 Consumer 구조 - 컴포지션 패턴
        # - 이유: 기본 EventConsumer(큐 처리)와 비즈니스 로직 분리
        # - 요구사항: System으로 동작하면서 EventConsumer 내장
        # - 구조: ExperienceConsumerSystem HAS-A EventConsumer
        self._event_consumer: EventConsumer[EnemyDeathEvent] = None  # 나중에 주입
