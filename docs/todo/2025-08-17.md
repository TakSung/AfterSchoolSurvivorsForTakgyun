...
class EventTunnelManager:
    ...
    # register_event_system() 메서드 제거
    # - 이유: 불필요한 복잡성, System이 직접 get_consumer() 호출하면 됨
    # - 장점: 의존성 방향 단순화, 타입 안전성 향상
...
**간단한 System HAS-A Consumer 구조 사용 예시**:
```python
# EventTunnelManager 사용 예시 - 단순한 직접 접근
tunnel_manager = EventTunnelManager()
experience_manager = ExperienceManager()

# 1. System이 생성자에서 직접 Consumer 가져오기
experience_system = ExperienceSystem(experience_manager, tunnel_manager)
# ↑ 내부에서 tunnel_manager.get_consumer(EnemyDeathEvent) 호출

# 2. Producer 가져오기 (다른 시스템에서 이벤트 발행용)
producer = tunnel_manager.get_producer(EnemyDeathEvent)

# 3. 다른 System에서 이벤트 생산 (예: ProjectileSystem)
...
# 4. ExperienceSystem이 자동으로 처리 (System.update() 호출 시)
...
# 5. 구조 확인 (등록 단계 불필요!)
print("=== 단순한 System HAS-A Consumer 구조 ===")
...
│
├── get_producer(event_type) → EventProducer    ← 다른 System이 호출
├── get_consumer(event_type) → EventConsumer    ← ExperienceSystem이 직접 호출
│
└── ExperienceSystem (비즈니스 로직)   ← systems/events/experience_system.py
    └── HAS-A: EventConsumer           ← 생성자에서 직접 가져옴 (단순함!)
```

**장점**:
- ✅ **중간 등록 단계 제거**: `register_event_system()` 불필요
- ✅ **타입 안전성**: `hasattr()` 런타임 체크 제거
- ✅ **단방향 의존성**: EventTunnelManager가 System을 몰라도 됨
- ✅ **단순성**: System이 필요할 때 직접 Consumer 가져옴
...
class ExperienceSystem(System):
    ...
    def __init__(
        ...
        """의존성 주입: 경험치 관리자 + 터널 매니저"""
        super().__init__(priority=20)  # Experience 처리는 후순위
        self._experience_manager = experience_manager
        
        # AI-NOTE : 2025-08-17 System이 직접 EventConsumer를 가져오는 단순한 구조
        # - 이유: 중간 등록 단계 제거로 복잡성 감소, 타입 안전성 향상  
        # - 구조: System → EventTunnelManager.get_consumer() → EventConsumer
        self._event_consumer: EventConsumer[EnemyDeathEvent] = tunnel_manager.get_consumer(EnemyDeathEvent)
