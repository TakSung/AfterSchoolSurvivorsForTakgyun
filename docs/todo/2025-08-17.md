...
**Producer-Consumer 연결 동작 예시**:
```python
# EventTunnelManager 사용 예시 - 공유 큐 기반 연결
tunnel_manager = EventTunnelManager()

# 1. 터널 생성 (공유 큐 포함)
producer, consumer = tunnel_manager.create_tunnel(EnemyDeathEvent)
...
success = producer.produce(death_event)  # 공유 큐에 추가

# 3. Consumer가 동일한 큐에서 이벤트 소비
next_event = consumer.consume_next()     # 공유 큐에서 제거
assert next_event.enemy_entity_id == "enemy_123"  # 동일한 이벤트 확인

# 4. 큐 상태 모니터링
stats = tunnel_manager.get_tunnel_stats()
print(f"Total events: {stats['total_queued_events']}")
```
