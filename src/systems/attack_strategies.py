...
class DirectionAttackStrategy(IAttackStrategy):
    ...
    def calculate_direction(
        ...
        **kwargs
        ...
        """플레이어가 바라보는 방향으로 공격"""
        logger.debug(f"DirectionAttackStrategy: calculating direction for angle {direction_angle}")
    ...
    def calculate_direction(
        ...
        **kwargs
        ...
        if direction_vector.magnitude == 0:
            logger.debug("WorldTargetAttackStrategy: Target at same position as start")
            return None  # 동일 위치면 공격 불가
        
        logger.debug(f"WorldTargetAttackStrategy: direction from {start_pos.x},{start_pos.y} to {target_pos.x},{target_pos.y}")
    ...
    def calculate_direction(
        ...
        **kwargs
        ...
        if not entity_manager:
            logger.warning("NearestEnemyAttackStrategy: No entity_manager provided")
            return None
        ...
        from ..components.position_component import PositionComponent
        target_pos = entity_manager.get_component(nearest_enemy, PositionComponent)
        if not target_pos:
            logger.warning("NearestEnemyAttackStrategy: Target enemy has no position component")
            return None
