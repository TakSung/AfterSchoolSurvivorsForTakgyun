    ...
    def detect_collisions(
        ...
        for i, entity1 in enumerate(entities):
            for entity2 in entities[i + 1:]:  # 중복 검사 방지
                ...
                if self._check_entity_collision(entity_manager, entity1, entity2):
...
class CollisionSystem(System):
    ...
    def __init__(
        self, 
        priority: int = 100, 
        detector: Optional[ICollisionDetector] = None
    ) -> None:
        """
    ...
    def update(self, entity_manager: 'EntityManager', delta_time: float) -> None:
        ...
        collision_pairs = self._detector.detect_collisions(entity_manager, entities)
    ...
    def set_collision_detector(self, detector: ICollisionDetector) -> None:
        ...
        logger.info(f"Collision detector changed to {detector.__class__.__name__}")
    ...
    def _handle_collision(
        ...
        # 양방향 충돌 마스크 확인
        can_collide = (
            col1.can_collide_with(col2.layer) or 
            col2.can_collide_with(col1.layer)
        )
        ...
        # 충돌 이벤트 처리
        self._process_collision_event(entity_manager, entity1, entity2, col1, col2)
    ...
    def _process_collision_event(
        ...
        if layer_pair == (CollisionLayer.PLAYER, CollisionLayer.ENEMY):
            self._handle_player_enemy_collision(entity_manager, entity1, entity2, col1, col2)
        elif layer_pair == (CollisionLayer.PLAYER, CollisionLayer.ITEM):
            self._handle_player_item_collision(entity_manager, entity1, entity2, col1, col2)
        elif layer_pair == (CollisionLayer.PROJECTILE, CollisionLayer.ENEMY):
            self._handle_projectile_enemy_collision(entity_manager, entity1, entity2, col1, col2)
        else:
            # 기본 충돌 처리 (로깅만)
            logger.debug(
    ...
    def _handle_player_enemy_collision(
        ...
        logger.info(f"Player-Enemy collision: player={entity1.id if col1.layer.value == 0 else entity2.id}")
    ...
    def _handle_player_item_collision(
        ...
        logger.info(f"Player-Item collision: item acquired")
    ...
    def _handle_projectile_enemy_collision(
        ...
        logger.info(f"Projectile-Enemy collision: damage applied")
