...
class ProjectileFactory:
    ...
    def create_projectile(
        ...
        try:
            ...
            from ..components.render_component import (
                RenderComponent,
                RenderLayer,
            )
            ...
            logger.debug(
                f'Created projectile entity: {projectile_entity.entity_id}'
            )
            ...
            for comp_type, comp_name in required_components:
                if not entity_manager.has_component(
                    projectile_entity, comp_type
                ):
                    logger.error(
                        f'Failed to add {comp_name} to projectile {projectile_entity.entity_id}'
                    )
                    entity_manager.destroy_entity(projectile_entity)
                    return None

            logger.info(
                f'Successfully created projectile {projectile_entity.entity_id} with all components'
            )
