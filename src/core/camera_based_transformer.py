...
class CameraBasedTransformer(ICoordinateTransformer):
    ...
    def __init__(
        ...
        self._camera_offset = camera_offset.copy() if camera_offset else Vector2.zero()
        self._zoom_level = max(0.1, zoom_level)
        self._cache_dirty = True
        self._transformation_matrix_cache: Optional[tuple[float, float, float, float, float, float]] = None
        self._inverse_matrix_cache: Optional[tuple[float, float, float, float, float, float]] = None
    ...
    def get_transformation_matrix(self) -> tuple[float, float, float, float, float, float]:
    ...
    def get_inverse_transformation_matrix(self) -> tuple[float, float, float, float, float, float]:
        if self._cache_dirty or self._inverse_matrix_cache is None:
            ...
            self._inverse_matrix_cache = (inv_scale, 0.0, inv_tx, 0.0, inv_scale, inv_ty)
        
        return self._inverse_matrix_cache
    
    def transform_multiple_points(self, world_positions: list[Vector2]) -> list[Vector2]:
    ...
    def get_cache_stats(self) -> dict[str, any]:
        return {
            ...
            "has_transform_matrix": self._transformation_matrix_cache is not None,
