...
class InfiniteCameraSystem(CameraSystem):
    ...
    def __init__(self, priority: int = 10):
        ...
        self.dead_zone_enabled = False  # 데드존 비활성화 상태로 시작 (무한 스크롤)
    ...
    def _update_camera_by_mouse_tracking(
        ...
        effective_dead_zone = self.original_dead_zone if self.dead_zone_enabled else 0.0
        ...
        new_offset_x = current_offset[0] - direction_x * move_speed * delta_time
        new_offset_y = current_offset[1] - direction_y * move_speed * delta_time
...
def main():
    ...
    pygame.display.set_caption("MapRenderSystem E2E Demo - Infinite Scroll Tiles")
    ...
    camera_system = InfiniteCameraSystem(priority=10)  # 무한 스크롤 카메라 시스템 사용
    ...
    while running:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_ESCAPE:
                    ...
                    print(f"Performance stats: {'ON' if show_performance else 'OFF'}")
                    ...
                    infinite_offset_active = camera_system.toggle_infinite_offset()
                    status = "ON (Infinite)" if infinite_offset_active else "OFF (Limited)"
        ...
        pygame.draw.line(screen, (255, 255, 100), (center_x, center_y), (end_x, end_y), 3)

        # Draw dead zone circle (if enabled)
        if camera_system.dead_zone_enabled:
            pygame.draw.circle(screen, (100, 100, 100), (center_x, center_y),
                              int(camera_system.original_dead_zone), 1)
        else:
        ...
        if frame_count % 60 == 0:  # Print every second
            ...
            if render_tiles:
                ...
                if center_tile is None and render_tiles:
                    center_tile = render_tiles[len(render_tiles)//2]  # 중간 타일 사용
                
                if center_tile:
                    print(f"DEBUG - Player: ({world_pos[0]:.1f}, {world_pos[1]:.1f}) | "
        ...
        if show_performance:
            display_performance_info(screen, movement_comp, camera_comp, map_render_system, camera_system)
        ...
        if fps_timer >= 1.0:  # Print FPS every second
            ...
            if show_performance:
                print(f"FPS: {current_fps:.1f} | Tiles: {map_render_system.get_tile_stats()['total_visible']}")
            frame_count = 0
            fps_timer = 0
...
def display_performance_info(screen, movement_comp, camera_comp, map_render_system, camera_system):
    ...
    # Performance header
    header_text = small_font.render("Performance Stats (Press P to toggle)", True, (255, 255, 0))
    screen.blit(header_text, (10, y_offset))
    y_offset += line_height

    # Dead zone status
    dead_zone_status = "ON" if camera_system.dead_zone_enabled else "OFF (Infinite)"
    color = (255, 200, 100) if camera_system.dead_zone_enabled else (100, 255, 200)
    text = font.render(f"Dead Zone: {dead_zone_status}", True, color)
    screen.blit(text, (10, y_offset))
    y_offset += line_height

    # Camera offset bounds status
    offset_status = "Infinite" if camera_system.infinite_offset_enabled else "Limited"
    offset_color = (100, 255, 200) if camera_system.infinite_offset_enabled else (255, 200, 100)
    ...
    world_pos = movement_comp.world_position
    text = font.render(f"Player: ({world_pos[0]:.1f}, {world_pos[1]:.1f})", True, (255, 255, 255))
    screen.blit(text, (10, y_offset))
    y_offset += line_height

    # Camera offset
    camera_offset = camera_comp.world_offset
    text = font.render(f"Camera: ({camera_offset[0]:.1f}, {camera_offset[1]:.1f})", True, (255, 255, 255))
    screen.blit(text, (10, y_offset))
    y_offset += line_height

    # Tile statistics (Task 17.3 verification)
    tile_stats = map_render_system.get_tile_stats()
    text = font.render(f"Visible Tiles: {tile_stats['total_visible']}", True, (100, 255, 100))
    screen.blit(text, (10, y_offset))
    y_offset += line_height

    text = font.render(f"New/Removed: {tile_stats['new_tiles']}/{tile_stats['removed_tiles']}", True, (200, 200, 255))
    screen.blit(text, (10, y_offset))
    y_offset += line_height
