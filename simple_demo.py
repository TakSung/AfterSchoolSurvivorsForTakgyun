...
class SimpleDemo:
    ...
    def _create_entities(self):
        ...
        self.entity_manager.add_component(player_entity, PlayerComponent(player_id=0))
        self.entity_manager.add_component(player_entity, PositionComponent(x=0.0, y=0.0))
        ...
        # 고정된 테스트 엔티티들
        positions = [
            (-100, -100), (100, -100), (-100, 100), (100, 100),  # 네 모서리
            (0, -150), (0, 150), (-150, 0), (150, 0),            # 십자 패턴
            (-200, -200), (200, 200), (-200, 200), (200, -200)   # 더 먼 모서리
        ]
        colors = [
            (255, 100, 100), (100, 255, 100), (255, 255, 100), (255, 100, 255),
            (100, 255, 255), (255, 200, 100), (200, 100, 255), (150, 150, 255),
            (255, 150, 150), (150, 255, 150), (255, 255, 150), (255, 150, 255)
        ]
        
        for i, (x, y) in enumerate(positions):
            entity = self.entity_manager.create_entity()
            self.entity_manager.add_component(entity, PositionComponent(x=float(x), y=float(y)))
            self.entity_manager.add_component(
                entity,
                RenderComponent(
                    color=colors[i],
        ...
        for x in range(-2, 3):
            for y in range(-2, 3):
                ...
                self.entity_manager.add_component(
                    entity,
                    PositionComponent(x=x * 200.0, y=y * 200.0)
                )
                self.entity_manager.add_component(
                    entity,
                    RenderComponent(
