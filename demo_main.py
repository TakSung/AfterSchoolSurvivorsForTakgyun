...
class EnhancedGameDemo:
    ...
    def _setup_systems(self) -> None:
        ...
        player_movement_system._screen_center = (self.screen_width // 2, self.screen_height // 2)
        self.system_orchestrator.register_system(player_movement_system, "player_movement")
    ...
    def _create_demo_entities(self) -> None:
        ...
        self.entity_manager.add_component(self.player_entity, player_render_comp)
        
        # 플레이어 회전 및 이동 컴포넌트
        self.entity_manager.add_component(
            self.player_entity,
            RotationComponent(angle=0.0)
        )
        self.entity_manager.add_component(
            self.player_entity,
    ...
    def _update(self, delta_time: float) -> None:
        ...
        for i, entity in enumerate(self.demo_entities):
            rotation_comp = self.entity_manager.get_component(entity, RotationComponent)
            if rotation_comp:
                # 각 엔티티마다 다른 속도로 회전
        ...
        # 시스템들 업데이트
        self.system_orchestrator.update_systems(self.entity_manager, delta_time)
    ...
    def _create_world_entities(self) -> None:
        ...
        for i in range(num_entities):
            ...
            color = self.colors['entity_colors'][i % len(self.colors['entity_colors'])]
    ...
    def _create_background_entities(self) -> None:
        ...
        for x in range(-grid_range, grid_range + 1, 2):
            for y in range(-grid_range, grid_range + 1, 2):
                ...
                self.entity_manager.add_component(
                    entity,
                    PositionComponent(x=world_x, y=world_y)
                )
    ...
    def _draw_world_grid(self) -> None:
        ...
        # 플레이어 위치 가져오기
        pos_comp = self.entity_manager.get_component(self.player_entity, PositionComponent)
        if not pos_comp:
            return
        ...
        for x in range(-grid_offset_x, self.screen_width + grid_size, grid_size):
        ...
        for y in range(-grid_offset_y, self.screen_height + grid_size, grid_size):
        ...
        if transformer:
            try:
                ...
                if (0 <= world_center.x <= self.screen_width and 
                    0 <= world_center.y <= self.screen_height):
                    pygame.draw.circle(
                        self.screen, 
                        (150, 150, 200), 
                    ...
                    # 중심점 라벨
                    center_text = self.font_small.render("(0,0)", True, (150, 150, 200))
                    self.screen.blit(center_text, (int(world_center.x) + 10, int(world_center.y) + 10))
            except:
                pass  # 좌표 변환 실패 시 무시
    ...
    def _draw_performance_stats(self) -> None:
        ...
        for i, text in enumerate(perf_texts):
            surface = self.font_medium.render(text, True, self.colors['ui_accent'])
    ...
    def _draw_coordinate_info(self) -> None:
        ...
        if pos_comp:
            ...
            for i, text in enumerate(coord_texts):
                surface = self.font_small.render(text, True, self.colors['ui_accent'])
                self._draw_text_with_background(surface, 10, start_y + i * 20)
    ...
    def _draw_text_with_background(self, text_surface: pygame.Surface, x: int, y: int) -> None:
        """배경과 함께 텍스트 렌더링."""
