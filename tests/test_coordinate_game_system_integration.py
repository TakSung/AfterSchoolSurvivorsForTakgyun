...
class TestCoordinateGameSystemIntegration:
    ...
    def test_적_AI_월드좌표_추적_화면렌더링_일치성_검증_성공_시나리오(self, game_environment):
        ...
        # 1. 적이 플레이어를 향해 이동했는지 확인 (월드 좌표)
        initial_world_distance = initial_data['world_distance']
        final_world_distance = final_data['world_distance']
        assert final_world_distance < initial_world_distance, (
            f"적이 플레이어에게 접근해야 함: {initial_world_distance} -> {final_world_distance}"
        )
        ...
        # 3. 적이 실제로 플레이어 방향으로 이동했는지 확인
        enemy_movement_distance = initial_data['enemy_world'].distance_to(final_data['enemy_world'])
        assert enemy_movement_distance > 20, f"적이 충분히 이동해야 함: {enemy_movement_distance}"
    ...
    def test_투사체_시스템_궤적_충돌감지_정확성_검증_성공_시나리오(self, game_environment):
        ...
        for frame in range(frames):
            ...
            # 예상 위치 계산 (직선 이동)
            expected_world_pos = Vector2(frame * 200.0 * delta_time, 0)
                ...
                with patch('pygame.event.get', return_value=[]):
                    ...
                    for frame in range(frames):
                        ...
                        stability_data.append({
                            ...
                            'player_world': player_world_pos.copy(),
                            'camera_offset': camera_offset.copy(),
                            'player_screen': player_screen_pos.copy(),
