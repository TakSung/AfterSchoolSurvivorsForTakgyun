...
class TestPlayerCameraIntegration:
    ...
    def test_플레이어_이동_시_카메라_역방향_추적_검증_성공_시나리오(
        ...
        system_orchestrator.register_system(player_movement_system, 'player_movement')
        ...
        expected_offset_x = 400 - updated_world_pos[0]  # 화면 중앙 - 플레이어 위치
    ...
    def test_마우스_방향_변경_시_플레이어_회전_및_카메라_추적_검증_성공_시나리오(
        ...
        expected_angle = math.atan2(150, 0)  # 450 - 300 = 150 (Y 방향)
    ...
    def test_데드존_내부_마우스_위치_시_플레이어_정지_검증_성공_시나리오(
        ...
        # Given - 데드존 내부 마우스 위치
        mock_pygame.get_init.return_value = True
        mock_pygame.mouse.get_pos.return_value = (405, 305)  # 중앙에서 5픽셀 이내
        ...
        assert abs(final_camera_offset[0] - initial_camera_offset[0]) < 0.001, \
               "데드존 내부에서는 카메라 X 오프셋이 변경되지 않아야 함"
        assert abs(final_camera_offset[1] - initial_camera_offset[1]) < 0.001, \
    ...
    def test_다중_업데이트_시_연속적인_플레이어_이동_및_카메라_추적_검증_성공_시나리오(
        ...
        actual_distance = math.sqrt(final_world_pos[0]**2 + final_world_pos[1]**2)
