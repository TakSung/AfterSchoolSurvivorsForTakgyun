...
class TestTimeManager:
    def test_시간_관리자_초기화_상태_확인_성공_시나리오(self) -> None:
        ...
        # Then - 초기 상태 확인
        assert time_manager.time_mode == TimeMode.VARIABLE_TIMESTEP, (
            '기본 시간 모드는 가변 시간 간격이어야 함'
        )
        assert time_manager.fixed_timestep == 1.0 / 60.0, (
            '기본 고정 시간 간격은 60 FPS여야 함'
        )
        ...
        assert time_manager.total_game_time == 0.0, (
            '초기 총 게임 시간은 0.0이어야 함'
        )
        assert not time_manager.is_paused, (
            '초기에는 일시정지 상태가 아니어야 함'
        )
    ...
    def test_시간_모드_전환_기능_검증_성공_시나리오(self) -> None:
        ...
        # Then - 모드 전환 확인
        assert time_manager.time_mode == TimeMode.FIXED_TIMESTEP, (
            '고정 시간 간격 모드로 전환되어야 함'
        )
        assert time_manager._accumulated_time == 0.0, (
            '모드 전환 시 누적 시간이 초기화되어야 함'
        )
    ...
    def test_시간_스케일링_기능_검증_성공_시나리오(self) -> None:
        ...
        time_manager.resume()
        assert time_manager.time_scale == 1.0, (
            '재개 시 time_scale은 1.0이어야 함'
        )
        assert not time_manager.is_paused, (
            '재개 후 일시정지 상태가 아니어야 함'
        )

        # When & Then - 슬로우모션
        time_manager.set_slow_motion(0.5)
        assert time_manager.time_scale == 0.5, (
            '슬로우모션 시 time_scale은 0.5여야 함'
        )

        # When & Then - 시간 스케일 리셋
        time_manager.reset_time_scale()
        assert time_manager.time_scale == 1.0, (
            '리셋 후 time_scale은 1.0이어야 함'
        )
    ...
    def test_고정_시간_간격_모드_업데이트_검증_성공_시나리오(self) -> None:
        ...
        time_manager = TimeManager(
            time_mode=TimeMode.FIXED_TIMESTEP, fixed_timestep=fixed_timestep
        )
        ...
        assert update_count == 0, (
            '고정 timestep 미만 시간에서는 업데이트가 호출되지 않아야 함'
        )
        ...
        assert update_count == 1, (
            '고정 timestep 시간에서는 한 번 업데이트가 호출되어야 함'
        )
    ...
    def test_가변_시간_간격_모드_업데이트_검증_성공_시나리오(self) -> None:
        ...
        assert update_callback.call_count == len(test_deltas), (
            '각 업데이트마다 콜백이 호출되어야 함'
        )

        # 마지막 델타 시간이 현재 델타 시간과 일치
        assert time_manager.delta_time == test_deltas[-1], (
            '마지막 델타 시간이 저장되어야 함'
        )
    ...
    def test_시간_스케일_적용_업데이트_검증_성공_시나리오(self) -> None:
        ...
        assert time_manager.delta_time == raw_delta * 1.0, (
            '정상 속도에서 델타 시간이 정확해야 함'
        )
        ...
        assert time_manager.delta_time == raw_delta * 0.5, (
            '슬로우모션에서 델타 시간이 절반이어야 함'
        )
        ...
        assert time_manager.delta_time == 0.0, (
            '일시정지에서 델타 시간이 0이어야 함'
        )
    ...
    def test_콜백_관리_기능_검증_성공_시나리오(self) -> None:
        ...
        assert stats['update_callbacks_count'] == 3, (
            '3개의 콜백이 등록되어야 함'
        )
    ...
    def test_보간_인자_계산_기능_검증_성공_시나리오(self) -> None:
        ...
        assert abs(interpolation_factor - 0.5) < 0.001, (
            '보간 인자가 0.5여야 함'
        )
        ...
        # Then - 보간 인자 0.75
        interpolation_factor = time_manager.get_interpolation_factor()
        assert abs(interpolation_factor - 0.75) < 0.001, (
            '보간 인자가 0.75여야 함'
        )
    ...
    def test_최대_프레임_시간_제한_검증_성공_시나리오(self) -> None:
        ...
        assert time_manager.delta_time == max_frame_time, (
            '델타 시간이 최대값으로 제한되어야 함'
        )
    ...
    def test_시간_통계_정보_제공_정확성_검증_성공_시나리오(self) -> None:
        ...
        time_manager = TimeManager(
            time_mode=TimeMode.FIXED_TIMESTEP, fixed_timestep=1.0 / 40.0
        )
        ...
        # Then - 통계 정보 확인
        assert stats['time_mode'] == '고정 시간 간격', (
            '시간 모드 표시명이 정확해야 함'
        )
        ...
        assert stats['fixed_timestep'] == 1.0 / 40.0, (
            '고정 timestep이 정확해야 함'
        )
        ...
        assert abs(stats['unscaled_delta_time'] - expected_unscaled) < 0.001, (
            '스케일되지 않은 델타 시간이 정확해야 함'
        )
    ...
    def test_시간_열거형_표시명_정확성_검증_성공_시나리오(self) -> None:
        ...
        assert TimeMode.FIXED_TIMESTEP.display_name == '고정 시간 간격', (
            '고정 timestep 표시명이 정확해야 함'
        )
        assert TimeMode.VARIABLE_TIMESTEP.display_name == '가변 시간 간격', (
            '가변 timestep 표시명이 정확해야 함'
        )
