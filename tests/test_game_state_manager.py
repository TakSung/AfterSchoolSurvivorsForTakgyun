...
class TestGameStateManager:
    def test_게임_상태_초기화_및_기본값_검증_성공_시나리오(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # Then - 초기 상태 검증
            assert manager.current_state == GameState.STOPPED, (
                '초기 상태는 STOPPED이어야 함'
            )
            assert manager.previous_state == GameState.STOPPED, (
                '이전 상태도 STOPPED이어야 함'
            )
            assert manager.is_stopped(), 'is_stopped()가 True를 반환해야 함'
            assert not manager.is_running(), (
                'is_running()이 False를 반환해야 함'
            )
            assert not manager.is_paused(), 'is_paused()가 False를 반환해야 함'

            # 기본 설정 검증
            display_config = manager.get_display_config()
            assert display_config['width'] == 1280, (
                '기본 해상도 너비가 1280이어야 함'
            )
            assert display_config['height'] == 720, (
                '기본 해상도 높이가 720이어야 함'
            )
            assert display_config['fps_target'] == 60, (
                '기본 FPS 목표가 60이어야 함'
            )

    def test_게임_상태_전환_유효성_및_콜백_호출_검증_성공_시나리오(
        self,
    ) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # STOPPED -> RUNNING
            assert manager.can_transition_to(GameState.RUNNING), (
                'STOPPED에서 RUNNING으로 전환 가능해야 함'
            )
            ...
            # 무효한 전환 테스트
            assert not manager.can_transition_to(GameState.STOPPED), (
                'RUNNING에서 STOPPED로 직접 전환 불가해야 함'
            )
    ...
    def test_설정_파일_로딩_저장_및_병합_검증_성공_시나리오(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # Then - 병합된 설정 검증
            assert manager.get_config('display.width') == 1920, (
                '로딩된 해상도 너비가 적용되어야 함'
            )
            assert manager.get_config('display.height') == 1080, (
                '로딩된 해상도 높이가 적용되어야 함'
            )
            assert manager.get_config('display.fps_target') == 60, (
                '기본값이 유지되어야 함'
            )
            assert manager.get_config('audio.master_volume') == 0.5, (
                '로딩된 볼륨이 적용되어야 함'
            )
            assert manager.get_config('audio.music_volume') == 0.8, (
                '기본 음악 볼륨이 유지되어야 함'
            )
            assert (
                manager.get_config('custom_section.custom_value') == 'test'
            ), '커스텀 설정이 추가되어야 함'
            ...
            # 먼저 변경사항 확인
            assert manager.get_config('display.vsync') is False, (
                '변경 직후 vsync 설정이 False여야 함'
            )
            assert manager.get_config('custom_section.new_value') == 42, (
                '변경 직후 커스텀 값이 42여야 함'
            )
            ...
            # 비동기 저장이 완료될 때까지 잠깐 대기
            time.sleep(0.05)  # 50ms 대기로 레이스 컨디션 방지

            # 새 인스턴스로 검증
            manager2 = GameStateManager(
                config_path=config_path, auto_save=False
            )
            assert manager2.get_config('display.vsync') is False, (
                '저장된 vsync 설정이 로딩되어야 함'
            )
            assert manager2.get_config('custom_section.new_value') == 42, (
                '저장된 커스텀 값이 로딩되어야 함'
            )
    ...
    def test_런타임_설정_변경_및_자동_저장_검증_성공_시나리오(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # Then - 즉시 반영 확인
            assert manager.get_config('display.width') == 2560, (
                '해상도 너비 변경이 즉시 반영되어야 함'
            )
            assert manager.get_config('audio.muted') is True, (
                '음소거 설정이 즉시 반영되어야 함'
            )
            assert manager.get_config('input.mouse_sensitivity') == 1.5, (
                '마우스 감도 변경이 즉시 반영되어야 함'
            )
            assert manager.get_config('gameplay.difficulty') == 'hard', (
                '난이도 변경이 즉시 반영되어야 함'
            )
            assert (
                manager.get_config('input.keyboard_bindings.pause') == 'space'
            ), '키 바인딩 변경이 즉시 반영되어야 함'
            assert (
                manager.get_config('custom.nested.deep.value') == 'deep_test'
            ), '깊은 중첩 설정이 즉시 반영되어야 함'

            # 기본값 테스트
            assert (
                manager.get_config('nonexistent.key', 'default') == 'default'
            ), '존재하지 않는 키는 기본값을 반환해야 함'
            ...
            assert manager2.get_config('display.width') == 2560, (
                '변경된 설정이 파일에 저장되어야 함'
            )
            assert (
                manager2.get_config('custom.nested.deep.value') == 'deep_test'
            ), '중첩된 설정도 파일에 저장되어야 함'
    ...
    def test_상태_토글_및_편의_메서드_검증_성공_시나리오(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert not manager.toggle_pause(), (
                'STOPPED 상태에서 토글은 실패해야 함'
            )
            assert manager.is_stopped(), '여전히 STOPPED 상태여야 함'

            # resume 실패 케이스 (PAUSED 상태가 아닐 때)
            assert not manager.resume(), (
                'STOPPED 상태에서 resume은 실패해야 함'
            )
    ...
    def test_설정_초기화_및_콜백_관리_검증_성공_시나리오(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert manager.get_config('display.width') == 1280, (
                '해상도가 기본값으로 복원되어야 함'
            )
            assert (
                manager.get_config('custom.value', 'not_found') == 'not_found'
            ), '커스텀 설정이 제거되어야 함'
    ...
    def test_스레드_안전성_및_동시성_검증_성공_시나리오(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            def worker(thread_id: int) -> None:
                try:
                    for i in range(100):
                        # 설정 변경
                        manager.set_config(
                            f'thread_{thread_id}.value_{i}', i * thread_id
                        )

                        # 설정 읽기
                        value = manager.get_config(
                            f'thread_{thread_id}.value_{i}'
                        )
            ...
            # Then - 결과 검증
            assert len(errors) == 0, (
                f'스레드 실행 중 오류가 발생하지 않아야 함: {errors}'
            )
            assert len(results) == 500, (
                '모든 설정 작업이 완료되어야 함 (5 threads × 100 operations)'
            )
            ...
            for thread_id, i, value in results:
                ...
                assert value == expected, (
                    f'스레드 {thread_id}의 값 {i}가 예상값 {expected}와 일치해야 함'
                )

            # 마지막 display.width 값이 유효한 범위에 있는지 확인
            final_width = manager.get_config('display.width')
            assert 1920 <= final_width <= 1924, (
                f'최종 해상도 너비가 유효 범위에 있어야 함: {final_width}'
            )
    ...
    def test_상태_정보_조회_및_디버깅_기능_검증_성공_시나리오(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # Then - 상태 정보 검증
            assert state_info['current_state'] == '일시정지', (
                '현재 상태가 정확해야 함'
            )
            assert state_info['previous_state'] == '실행 중', (
                '이전 상태가 정확해야 함'
            )
            assert state_info['state_callbacks_count']['실행 중'] == 1, (
                'RUNNING 콜백 수가 정확해야 함'
            )
            assert state_info['state_callbacks_count']['일시정지'] == 1, (
                'PAUSED 콜백 수가 정확해야 함'
            )
            assert state_info['transition_callbacks_count'] == 1, (
                '전환 콜백 수가 정확해야 함'
            )
            assert str(config_path) in state_info['config_path'], (
                '설정 파일 경로가 포함되어야 함'
            )
            assert state_info['auto_save'] is False, (
                'auto_save 설정이 정확해야 함'
            )
            ...
            assert isinstance(display_config, dict), (
                'display 설정이 dict 타입이어야 함'
            )
            assert isinstance(audio_config, dict), (
                'audio 설정이 dict 타입이어야 함'
            )
            assert isinstance(input_config, dict), (
                'input 설정이 dict 타입이어야 함'
            )
            assert isinstance(gameplay_config, dict), (
                'gameplay 설정이 dict 타입이어야 함'
            )

            assert 'width' in display_config, (
                'display 설정에 width가 있어야 함'
            )
            assert 'master_volume' in audio_config, (
                'audio 설정에 master_volume이 있어야 함'
            )
            assert 'mouse_sensitivity' in input_config, (
                'input 설정에 mouse_sensitivity가 있어야 함'
            )
            assert 'difficulty' in gameplay_config, (
                'gameplay 설정에 difficulty가 있어야 함'
            )
