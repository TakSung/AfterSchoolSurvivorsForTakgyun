...
class TestStateHandler:
    def test_상태_디스패처_핸들러_등록_및_호출_검증_성공_시나리오(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert len(running_handler.input_calls) == 1, "RUNNING 핸들러가 호출되어야 함"
            assert len(paused_handler.input_calls) == 0, "PAUSED 핸들러는 호출되지 않아야 함"
            assert running_handler.input_calls[0] == test_event, "올바른 이벤트가 전달되어야 함"

            # 렌더링 처리
            test_renderer = MagicMock()
            dispatcher.handle_rendering(test_renderer)
            assert len(running_handler.render_calls) == 1, "RUNNING 핸들러 렌더링이 호출되어야 함"
            assert len(paused_handler.render_calls) == 0, "PAUSED 핸들러 렌더링은 호출되지 않아야 함"
            assert running_handler.render_calls[0] == test_renderer, "올바른 렌더러가 전달되어야 함"

            # 업데이트 처리
            dispatcher.update(0.016)
            assert len(running_handler.update_calls) == 1, "RUNNING 핸들러 업데이트가 호출되어야 함"
            assert len(paused_handler.update_calls) == 0, "PAUSED 핸들러 업데이트는 호출되지 않아야 함"
            assert running_handler.update_calls[0] == 0.016, "올바른 delta_time이 전달되어야 함"
            ...
            dispatcher.update(0.016)
            assert len(running_handler.update_calls) == 1, "RUNNING 핸들러 업데이트 호출 수가 변하지 않아야 함"
            assert len(paused_handler.update_calls) == 0, "PAUSED 상태에서는 업데이트가 호출되지 않아야 함"

            # 하지만 렌더링은 호출되어야 함
            dispatcher.handle_rendering(test_renderer)
            assert len(paused_handler.render_calls) == 1, "PAUSED 핸들러 렌더링이 호출되어야 함"
    ...
    def test_상태_프로세서_등록_및_체인_호출_검증_성공_시나리오(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            dispatcher.add_render_processor(GameState.RUNNING, render_processor1)
            dispatcher.add_render_processor(GameState.RUNNING, render_processor2)

            dispatcher.add_update_processor(GameState.RUNNING, update_processor1)
            dispatcher.add_update_processor(GameState.RUNNING, update_processor2)
            ...
            assert input_calls[0] == "processor1_test", "첫 번째 프로세서가 호출되어야 함"
            assert input_calls[1] == "processor2_test", "두 번째 프로세서가 호출되어야 함"

            # 렌더링 처리 (모든 프로세서 호출)
            dispatcher.handle_rendering(MagicMock())
            assert len(render_calls) == 2, "모든 렌더 프로세서가 호출되어야 함"
            assert render_calls[0] == "render1", "첫 번째 렌더 프로세서가 호출되어야 함"
            assert render_calls[1] == "render2", "두 번째 렌더 프로세서가 호출되어야 함"

            # 업데이트 처리 (모든 프로세서 호출)
            dispatcher.update(0.032)
            assert len(update_calls) == 2, "모든 업데이트 프로세서가 호출되어야 함"
            assert update_calls[0] == "update1_0.032", "첫 번째 업데이트 프로세서가 호출되어야 함"
            assert update_calls[1] == "update2_0.032", "두 번째 업데이트 프로세서가 호출되어야 함"
    ...
    def test_프로세서_등록_해제_및_정리_검증_성공_시나리오(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert "proc1" not in calls, "제거된 processor1은 호출되지 않아야 함"
            ...
            assert len(calls) == 0, "정리 후에는 어떤 프로세서도 호출되지 않아야 함"
            ...
            assert len(calls) == 0, "전체 정리 후에는 어떤 상태에서도 프로세서가 호출되지 않아야 함"
    ...
    def test_핸들러와_프로세서_혼합_사용_검증_성공_시나리오(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            dispatcher.handle_input(MockEvent("test"))
            assert execution_order[-1] == "handler_success", "핸들러가 마지막에 실행되어야 함"
            assert "processor_input" not in execution_order, "핸들러가 True 반환 시 프로세서는 호출되지 않아야 함"
            ...
            assert "processor_input" in execution_order, "핸들러가 False 반환 시 프로세서가 호출되어야 함"
            assert execution_order.index("handler_fail") < execution_order.index("processor_input"), "핸들러가 프로세서보다 먼저 실행되어야 함"

            execution_order.clear()

            # 렌더링과 업데이트는 항상 모두 호출
            dispatcher.add_render_processor(GameState.RUNNING, render_processor)
            dispatcher.add_update_processor(GameState.RUNNING, update_processor)

            dispatcher.handle_rendering(MagicMock())
            assert "handler_render" in execution_order, "핸들러 렌더링이 호출되어야 함"
            assert "processor_render" in execution_order, "프로세서 렌더링도 호출되어야 함"

            execution_order.clear()

            dispatcher.update(0.016)
            assert "handler_update" in execution_order, "핸들러 업데이트가 호출되어야 함"
            assert "processor_update" in execution_order, "프로세서 업데이트도 호출되어야 함"
    ...
    def test_예외_상황_처리_및_안정성_검증_성공_시나리오(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            dispatcher.add_input_processor(GameState.RUNNING, exception_input_processor)
            dispatcher.add_input_processor(GameState.RUNNING, safe_input_processor)
            dispatcher.add_render_processor(GameState.RUNNING, exception_render_processor)
            dispatcher.add_render_processor(GameState.RUNNING, safe_render_processor)
            dispatcher.add_update_processor(GameState.RUNNING, exception_update_processor)
            dispatcher.add_update_processor(GameState.RUNNING, safe_update_processor)
            ...
            assert "handler_input_start" in execution_log, "핸들러가 실행을 시작했어야 함"
            assert "processor_input_exception" in execution_log, "첫 번째 프로세서가 실행되었어야 함"
            assert "processor_input_safe" in execution_log, "예외 후에도 안전한 프로세서가 실행되어야 함"
            assert result is True, "안전한 프로세서가 True를 반환했으므로 전체 결과는 True여야 함"
            ...
            assert "handler_render_start" in execution_log, "핸들러 렌더링이 시작했어야 함"
            assert "processor_render_exception" in execution_log, "예외 프로세서가 실행되었어야 함"
            assert "processor_render_safe" in execution_log, "예외 후에도 안전한 프로세서가 실행되어야 함"
            ...
            assert "handler_update_start" in execution_log, "핸들러 업데이트가 시작했어야 함"
            assert "processor_update_exception" in execution_log, "예외 프로세서가 실행되었어야 함"
            assert "processor_update_safe" in execution_log, "예외 후에도 안전한 프로세서가 실행되어야 함"

            # 시스템이 여전히 정상 동작하는지 확인
            assert state_manager.is_running(), "예외 발생에도 불구하고 상태 관리자는 정상 동작해야 함"
