...
import pygame
from src.systems.render_system import RenderSystem, RenderLayer, RenderableSprite, LayeredSpriteGroup
from src.utils.vector2 import Vector2


class TestRenderLayer:
    def test_렌더링_레이어_열거형_표시명_정확성_검증_성공_시나리오(self) -> None:
        ...
        assert RenderLayer.BACKGROUND.display_name == "배경", "BACKGROUND 표시명이 정확해야 함"
        assert RenderLayer.GROUND.display_name == "지면", "GROUND 표시명이 정확해야 함"
        assert RenderLayer.ENTITIES.display_name == "엔티티", "ENTITIES 표시명이 정확해야 함"
        assert RenderLayer.PROJECTILES.display_name == "발사체", "PROJECTILES 표시명이 정확해야 함"
        assert RenderLayer.EFFECTS.display_name == "이펙트", "EFFECTS 표시명이 정확해야 함"
...
class TestRenderableSprite:
    ...
    def test_렌더링_가능_스프라이트_초기화_상태_확인_성공_시나리오(self) -> None:
        ...
        # Then - 초기 상태 확인
        assert sprite.layer == RenderLayer.ENTITIES, "기본 레이어는 ENTITIES여야 함"
        assert sprite.world_position == Vector2(), "초기 월드 위치는 (0,0)이어야 함"
        ...
        # Then - 레이어 확인
        assert ui_sprite.layer == RenderLayer.UI, "지정한 레이어로 설정되어야 함"
    ...
    def test_스프라이트_월드_위치_설정_검증_성공_시나리오(self) -> None:
        ...
        # Then - 위치 설정 확인
        assert sprite.world_position == new_position, "월드 위치가 설정되어야 함"
...
class TestLayeredSpriteGroup:
    ...
    def test_레이어드_스프라이트_그룹_초기화_상태_확인_성공_시나리오(self) -> None:
        ...
        # Then - 초기 상태 확인
        assert sprite_group.get_total_sprite_count() == 0, "초기 스프라이트 개수는 0이어야 함"
        
        for layer in RenderLayer:
            assert sprite_group.get_layer_sprite_count(layer) == 0, f"{layer.display_name} 레이어가 비어있어야 함"
    ...
    def test_스프라이트_추가_제거_기능_검증_성공_시나리오(self) -> None:
        ...
        # Then - 추가 확인
        assert sprite_group.get_total_sprite_count() == 2, "총 스프라이트 개수가 2개여야 함"
        assert sprite_group.get_layer_sprite_count(RenderLayer.ENTITIES) == 1, "엔티티 레이어에 1개 스프라이트가 있어야 함"
        assert sprite_group.get_layer_sprite_count(RenderLayer.UI) == 1, "UI 레이어에 1개 스프라이트가 있어야 함"
        
        # When - 스프라이트 제거
        sprite_group.remove_sprite(entity_sprite)
        
        # Then - 제거 확인
        assert sprite_group.get_total_sprite_count() == 1, "총 스프라이트 개수가 1개여야 함"
        assert sprite_group.get_layer_sprite_count(RenderLayer.ENTITIES) == 0, "엔티티 레이어가 비어있어야 함"
        assert sprite_group.get_layer_sprite_count(RenderLayer.UI) == 1, "UI 레이어에 1개 스프라이트가 남아있어야 함"
    ...
    def test_스프라이트_레이어_이동_기능_검증_성공_시나리오(self) -> None:
        ...
        # Then - 이동 확인
        assert sprite.layer == RenderLayer.EFFECTS, "스프라이트의 레이어가 변경되어야 함"
        assert sprite_group.get_layer_sprite_count(RenderLayer.ENTITIES) == 0, "기존 레이어가 비어있어야 함"
        assert sprite_group.get_layer_sprite_count(RenderLayer.EFFECTS) == 1, "새 레이어에 스프라이트가 있어야 함"
    ...
    def test_레이어별_클리어_기능_검증_성공_시나리오(self) -> None:
        ...
        # Then - 특정 레이어만 클리어 확인
        assert sprite_group.get_layer_sprite_count(RenderLayer.ENTITIES) == 0, "엔티티 레이어가 비어있어야 함"
        assert sprite_group.get_layer_sprite_count(RenderLayer.UI) == 1, "UI 레이어는 남아있어야 함"
        assert sprite_group.get_total_sprite_count() == 1, "총 1개 스프라이트가 남아있어야 함"
        
        # When - 전체 클리어
        sprite_group.clear_all()
        
        # Then - 전체 클리어 확인
        assert sprite_group.get_total_sprite_count() == 0, "모든 스프라이트가 제거되어야 함"
        for layer in RenderLayer:
            assert sprite_group.get_layer_sprite_count(layer) == 0, f"{layer.display_name} 레이어가 비어있어야 함"
...
class TestRenderSystem:
    ...
    def test_렌더링_시스템_초기화_상태_확인_성공_시나리오(self, mock_set_mode) -> None:
        ...
        assert render_system.screen_size == Vector2(800, 600), "화면 크기가 정확해야 함"
        assert render_system.background_color == (0, 0, 0), "기본 배경색이 검은색이어야 함"
        assert render_system.camera_transformer is None, "초기에는 카메라 변환기가 없어야 함"

    @patch('pygame.display.set_mode')
    def test_렌더링_시스템_스프라이트_관리_기능_검증_성공_시나리오(self, mock_set_mode) -> None:
        ...
        assert stats['layer_counts']['엔티티'] == 1, "엔티티 레이어에 1개 스프라이트가 있어야 함"
        assert stats['layer_counts']['UI'] == 1, "UI 레이어에 1개 스프라이트가 있어야 함"
        ...
        assert stats['layer_counts']['엔티티'] == 0, "엔티티 레이어가 비어있어야 함"

    @patch('pygame.display.set_mode')
    def test_렌더링_시스템_콜백_관리_기능_검증_성공_시나리오(self, mock_set_mode) -> None:
        ...
        # Then - 콜백 개수 확인
        stats = render_system.get_render_stats()
        assert stats['update_callbacks_count'] == 1, "업데이트 콜백이 1개 등록되어야 함"
        assert stats['pre_render_callbacks_count'] == 1, "pre-render 콜백이 1개 등록되어야 함"
        assert stats['post_render_callbacks_count'] == 1, "post-render 콜백이 1개 등록되어야 함"
        ...
        # Then - 콜백 제거 확인
        stats = render_system.get_render_stats()
        assert stats['update_callbacks_count'] == 0, "업데이트 콜백이 제거되어야 함"

    @patch('pygame.display.set_mode')  
    def test_렌더링_시스템_업데이트_기능_검증_성공_시나리오(self, mock_set_mode) -> None:
    ...
    @patch('pygame.display.set_mode')
    def test_렌더링_시스템_화면_클리어_기능_검증_성공_시나리오(self, mock_set_mode) -> None:
        ...
        render_system = RenderSystem(mock_surface, background_color=(255, 0, 0))
    ...
    @patch('pygame.display.set_mode')
    def test_렌더링_시스템_렌더링_기능_검증_성공_시나리오(self, mock_set_mode) -> None:
    ...
    @patch('pygame.display.set_mode')
    def test_렌더링_시스템_통계_정보_제공_정확성_검증_성공_시나리오(self, mock_set_mode) -> None:
        ...
        assert stats['background_color'] == (128, 128, 128), "배경색이 정확해야 함"
        assert stats['dirty_rects_tracking'], "dirty rect 추적이 활성화되어야 함"
        assert stats['update_callbacks_count'] == 1, "업데이트 콜백 개수가 정확해야 함"
        assert stats['has_camera_transformer'] is False, "카메라 변환기가 없어야 함"
        
        # 레이어별 통계 확인
        assert stats['layer_counts']['엔티티'] == 1, "엔티티 레이어 개수가 정확해야 함"
        assert stats['layer_counts']['UI'] == 1, "UI 레이어 개수가 정확해야 함"
        assert stats['layer_counts']['배경'] == 0, "배경 레이어가 비어있어야 함"

    @patch('pygame.display.set_mode')
    def test_렌더링_시스템_더티_렉트_추적_기능_검증_성공_시나리오(self, mock_set_mode) -> None:
        ...
        # Then - 초기 상태 확인
        stats = render_system.get_render_stats()
        assert stats['dirty_rects_tracking'], "더티 렉트 추적이 활성화되어야 함"
        assert stats['dirty_rects_count'] == 0, "초기 더티 렉트 개수는 0이어야 함"
        ...
        # Then - 비활성화 확인
        stats = render_system.get_render_stats()
        assert not stats['dirty_rects_tracking'], "더티 렉트 추적이 비활성화되어야 함"