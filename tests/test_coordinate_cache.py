...
from src.core.coordinate_cache import (
    CacheKey, CacheStats, LRUCache, CoordinateTransformCache
)
...
class TestCacheKey:
    def test_캐시_키_생성_및_해싱_정확성_검증_성공_시나리오(self) -> None:
        ...
        assert hash(key1) == hash(key2), "동일한 좌표의 키는 같은 해시를 가져야 함"
    ...
    def test_캐시_키_부동소수점_허용오차_검증_성공_시나리오(self) -> None:
        ...
        key1 = CacheKey(100.0, 200.0, 10.0, 20.0, 1.5, 800.0, 600.0)
        key2 = CacheKey(100.0005, 200.0002, 10.0001, 20.0003, 1.5001, 800.0, 600.0)
...
class TestCoordinateTransformCache:
    def test_좌표_변환_캐시_기본_동작_검증_성공_시나리오(self) -> None:
        ...
        # When - 캐시에 저장
        cache.put_world_to_screen(world_pos, camera_offset, zoom_level, screen_size, screen_result)
        
        # When - 캐시에서 조회
        cached_result = cache.get_world_to_screen(world_pos, camera_offset, zoom_level, screen_size)
        
        # Then - 저장된 값 정확히 조회
        assert cached_result is not None, "캐시된 결과가 존재해야 함"
        assert cached_result.distance_to(screen_result) < 0.001, "캐시된 결과가 정확해야 함"
        
        # When - 다른 좌표로 조회
        other_result = cache.get_world_to_screen(Vector2(200, 100), camera_offset, zoom_level, screen_size)
    ...
    def test_좌표_변환_캐시_통계_추적_검증_성공_시나리오(self) -> None:
        ...
        cache.put_world_to_screen(world_pos, camera_offset, zoom_level, screen_size, screen_result)
        cache.get_world_to_screen(world_pos, camera_offset, zoom_level, screen_size)  # 히트
        cache.get_world_to_screen(world_pos, camera_offset, zoom_level, screen_size)  # 히트
        cache.get_world_to_screen(Vector2(200, 100), camera_offset, zoom_level, screen_size)  # 미스
        ...
        assert abs(stats['total']['hit_rate'] - 2/3) < 0.001, "히트율이 정확해야 함"
    ...
    def test_좌표_변환_캐시_비활성화_동작_검증_성공_시나리오(self) -> None:
        ...
        # When - 조회 시도
        result = cache.get_world_to_screen(world_pos, camera_offset, zoom_level, screen_size)
...
class TestCachedCameraTransformer:
    def test_캐시_적용_변환기_기본_동작_검증_성공_시나리오(self) -> None:
        ...
        """
        # Given - 캐시 적용 변환기 생성
        transformer = CachedCameraTransformer(Vector2(800, 600), cache_size=100)
        world_pos = Vector2(100, 50)
        ...
        # Then - 결과 일치 확인
        assert result1.distance_to(result2) < 0.001, "캐시된 결과가 원본과 일치해야 함"
    ...
    def test_캐시_무효화_동작_검증_성공_시나리오(self) -> None:
        ...
        assert updated_stats['total']['misses'] > initial_stats['total']['misses'], "캐시 무효화로 새로운 미스가 발생해야 함"
    ...
    def test_캐시_성능_벤치마크_검증_성공_시나리오(self) -> None:
        ...
        # When - 성능 벤치마크 실행
        benchmark_result = transformer.benchmark_cache_performance(test_positions, iterations=10)
        
        # Then - 벤치마크 결과 구조 확인
        assert 'test_config' in benchmark_result, "테스트 설정 정보가 있어야 함"
        assert 'performance' in benchmark_result, "성능 측정 결과가 있어야 함"
        assert 'cache_stats' in benchmark_result, "캐시 통계가 있어야 함"
        
        # Then - 기본적인 성능 지표 확인
        assert benchmark_result['performance']['no_cache_time'] > 0, "캐시 없는 실행 시간이 측정되어야 함"
        assert benchmark_result['performance']['cache_time'] > 0, "캐시 있는 실행 시간이 측정되어야 함"
        assert benchmark_result['test_config']['total_operations'] == 100, "총 연산 수가 정확해야 함"