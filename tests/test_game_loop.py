...
def game_loop(mock_pygame_init: MagicMock, mock_game_state_manager: MagicMock) -> GameLoop:
...
class TestGameLoopRefactored:
    ...
    def test_gameloop_initialization_with_none_manager_fails(self) -> None:
        ...
        with pytest.raises(AssertionError, match="GameStateManager must be provided."):
    ...
    def test_callbacks_called_correctly_in_running_state(
        self, mock_event_get: MagicMock, mock_clock: MagicMock, game_loop: GameLoop, mock_game_state_manager: MagicMock
    ) -> None:
    ...
    def test_quit_event_calls_manager_stop(
        self, mock_clock: MagicMock, mock_event_get: MagicMock, game_loop: GameLoop, mock_game_state_manager: MagicMock
    ) -> None:
    ...
    def test_shutdown_calls_manager_stop_and_pygame_quit(
        self, mock_pygame_quit: MagicMock, game_loop: GameLoop, mock_game_state_manager: MagicMock
    ) -> None:
