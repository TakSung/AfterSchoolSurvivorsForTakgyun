...
class TestCoordinateSpace:
    def test_좌표공간_열거형_값_확인_성공_시나리오(self) -> None:
        ...
        assert CoordinateSpace.WORLD.value == 0, "WORLD 좌표공간 값이 0이어야 함"
        assert CoordinateSpace.SCREEN.value == 1, "SCREEN 좌표공간 값이 1이어야 함"
        
        # When & Then - 표시명 검증
        assert CoordinateSpace.WORLD.display_name == "월드 좌표", "WORLD 표시명이 정확해야 함"
        assert CoordinateSpace.SCREEN.display_name == "스크린 좌표", "SCREEN 표시명이 정확해야 함"
...
class TestICoordinateTransformer:
    def test_좌표_변환_인터페이스_기본_동작_확인_성공_시나리오(self) -> None:
        ...
        same_space_result = transformer.transform(test_position, CoordinateSpace.WORLD, CoordinateSpace.WORLD)
        assert same_space_result == test_position, "동일 좌표 공간 변환 시 같은 값이어야 함"
        assert same_space_result is not test_position, "동일 좌표 공간 변환 시 새로운 객체여야 함"
    ...
    def test_월드_스크린_좌표_변환_정확성_검증_성공_시나리오(self) -> None:
        ...
        # Then - 왕복 변환 정확성 검증
        assert world_pos.distance_to(back_to_world) < 0.001, "왕복 변환 시 좌표가 일치해야 함"
    ...
    def test_카메라_오프셋_적용_좌표_변환_검증_성공_시나리오(self) -> None:
        ...
        # Then - 오프셋 적용 확인
        assert transformer.get_camera_offset() == camera_offset, "카메라 오프셋이 정확히 설정되어야 함"
        expected_difference = screen_pos_no_offset - screen_pos_with_offset
        assert expected_difference.distance_to(Vector2(100, 50)) < 0.001, "카메라 오프셋이 올바르게 적용되어야 함"
    ...
    def test_화면_크기_변경_좌표_변환_검증_성공_시나리오(self) -> None:
        ...
        # Then - 화면 크기 변경 확인
        assert transformer.screen_size == new_screen_size, "화면 크기가 정확히 설정되어야 함"
        # 월드 원점이 새로운 화면 중심에 매핑되어야 함
        expected_center = new_screen_size / 2
        assert screen_pos_1024x768.distance_to(expected_center) < 0.001, "새로운 화면 중심으로 변환되어야 함"
    ...
    def test_화면_가시성_검사_정확성_검증_성공_시나리오(self) -> None:
        ...
        assert transformer.is_point_visible(center_world), "화면 중심은 가시 영역이어야 함"
        
        # When & Then - 화면 밖 점 가시성 검증 (비가시)
        far_world = Vector2(1000, 1000)  # 화면 밖
        assert not transformer.is_point_visible(far_world), "화면 밖 점은 비가시 영역이어야 함"
        
        # When & Then - 마진 포함 가시성 검증
        edge_world = Vector2(450, 350)  # 화면 경계 근처
        assert transformer.is_point_visible(edge_world, margin=100), "마진 포함 시 경계 근처 점은 가시여야 함"
    ...
    def test_지원하지_않는_좌표_변환_예외_처리_실패_시나리오(self) -> None:
        ...
        # 미래 확장을 위한 테스트 구조 준비
        try:
            result = transformer.transform(test_position, CoordinateSpace.WORLD, CoordinateSpace.SCREEN)
    ...
    def test_캐시_무효화_동작_확인_성공_시나리오(self) -> None:
        ...
        # Then - 캐시 무효화 플래그 확인
        assert transformer._cache_dirty, "카메라 오프셋 변경 시 캐시가 무효화되어야 함"
        ...
        # Then - 캐시 무효화 플래그 확인
        assert transformer._cache_dirty, "줌 레벨 변경 시 캐시가 무효화되어야 함"