...
class TestEnemyDeathEvent:
    ...
    def test_적_사망_이벤트_생성_및_기본_속성_검증_성공_시나리오(self) -> None:
        ...
        assert event.event_type == EventType.ENEMY_DEATH, '이벤트 타입이 ENEMY_DEATH여야 함'
        assert event.timestamp > 0, '타임스탬프가 설정되어야 함'
        assert event.created_at is not None, 'created_at이 설정되어야 함'
        assert isinstance(event.created_at, datetime), 'created_at이 datetime 타입이어야 함'
    ...
    def test_적_사망_이벤트_데이터_검증_성공_시나리오(self) -> None:
        ...
        for enemy_id in valid_ids:
            ...
            # Then - 검증 통과 확인
            assert event.validate() is True, f'유효한 ID {enemy_id}에 대해 검증이 통과해야 함'
    ...
    def test_엔티티_객체로부터_이벤트_생성_성공_시나리오(self) -> None:
        ...
        # Then - 올바른 생성 확인
        assert event.enemy_entity_id == "enemy_456", '엔티티 ID가 올바르게 추출되어야 함'
        assert event.validate() is True, '생성된 이벤트가 유효해야 함'
        assert event.event_type == EventType.ENEMY_DEATH, '이벤트 타입이 올바르게 설정되어야 함'
    ...
    def test_커스텀_타임스탬프_이벤트_생성_검증_성공_시나리오(self) -> None:
        ...
        # Then - 커스텀 값 확인
        assert event.timestamp == custom_timestamp, '커스텀 타임스탬프가 설정되어야 함'
        assert event.created_at == custom_datetime, '커스텀 created_at이 설정되어야 함'
        assert event.enemy_entity_id == enemy_id, '적 ID가 올바르게 설정되어야 함'
    ...
    def test_이벤트_나이_계산_정확성_검증_성공_시나리오(self) -> None:
        ...
        # Given - 특정 시간의 이벤트
        base_time = 1000.0
        event = EnemyDeathEvent.create_from_id("enemy_age_test", timestamp=base_time)
    ...
    def test_이벤트_만료_검사_로직_검증_성공_시나리오(self) -> None:
        ...
        # Given - 과거 시간의 이벤트
        old_time = 1000.0
        event = EnemyDeathEvent.create_from_id("enemy_expiry_test", timestamp=old_time)

        # When & Then - 만료되지 않은 경우
        current_time = old_time + 2.0
        max_age = 5.0
        assert not event.is_expired(max_age, current_time), '만료되지 않아야 함'
    ...
    def test_이벤트_타입_일관성_검증_성공_시나리오(self) -> None:
        ...
        assert event1.event_type == EventType.ENEMY_DEATH, '첫 번째 이벤트 타입 확인'
        assert event2.event_type == EventType.ENEMY_DEATH, '두 번째 이벤트 타입 확인'
        assert event1.event_type == event2.event_type, '모든 이벤트 타입이 동일해야 함'
    ...
    def test_get_enemy_id_메서드_동작_검증_성공_시나리오(self) -> None:
        ...
        # Then - 올바른 ID 반환 확인
        assert retrieved_id == enemy_id, 'get_enemy_id가 올바른 ID를 반환해야 함'
