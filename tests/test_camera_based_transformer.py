...
class TestCameraBasedTransformer:
    def test_카메라_변환기_초기화_정확성_검증_성공_시나리오(self) -> None:
        ...
        # Then - 초기 상태 확인
        assert transformer.screen_size == screen_size, "화면 크기가 정확히 설정되어야 함"
        assert transformer.get_camera_offset() == Vector2.zero(), "기본 카메라 오프셋은 영벡터여야 함"
        assert transformer.zoom_level == 1.0, "기본 줌 레벨은 1.0이어야 함"
        
        # Given & When - 모든 매개변수로 변환기 생성
        camera_offset = Vector2(100, 50)
        zoom_level = 2.0
        transformer_full = CameraBasedTransformer(screen_size, camera_offset, zoom_level)
        
        # Then - 설정된 값 확인
        assert transformer_full.get_camera_offset() == camera_offset, "카메라 오프셋이 정확히 설정되어야 함"
        assert transformer_full.zoom_level == zoom_level, "줌 레벨이 정확히 설정되어야 함"
    ...
    def test_기본_좌표_변환_정확성_검증_성공_시나리오(self) -> None:
        ...
        assert screen_pos == expected_center, "월드 원점이 화면 중심으로 변환되어야 함"
        
        # When & Then - 왕복 변환 정확성 검증
        back_to_world = transformer.screen_to_world(screen_pos)
        assert world_origin.distance_to(back_to_world) < 0.001, "왕복 변환 시 좌표가 일치해야 함"
    ...
    def test_카메라_오프셋_적용_변환_검증_성공_시나리오(self) -> None:
        ...
        assert offset_difference.distance_to(Vector2(50, 30)) < 0.001, "카메라 오프셋이 올바르게 적용되어야 함"
    ...
    def test_줌_레벨_적용_변환_검증_성공_시나리오(self) -> None:
        ...
        assert expected_offset_2.distance_to(offset_2) < 0.001, "줌 레벨에 따른 스케일링이 정확해야 함"
    ...
    def test_화면_크기_변경_변환_검증_성공_시나리오(self) -> None:
        ...
        assert screen_pos_1024x768.distance_to(expected_new_center) < 0.001, "새로운 화면 중심으로 변환되어야 함"
    ...
    def test_변환_매트릭스_캐싱_동작_검증_성공_시나리오(self) -> None:
        ...
        # Then - 캐시 동작 확인
        assert matrix1 == matrix2, "동일한 설정에서 같은 매트릭스가 반환되어야 함"
        assert cache_stats1["has_transform_matrix"], "첫 조회 후 매트릭스 캐시가 존재해야 함"
        assert cache_stats2["has_transform_matrix"], "두 번째 조회에서도 캐시가 유지되어야 함"
        ...
        # Then - 캐시 무효화 확인
        assert cache_stats3["cache_dirty"], "설정 변경 시 캐시가 무효화되어야 함"
    ...
    def test_다중_좌표_변환_성능_최적화_검증_성공_시나리오(self) -> None:
        ...
        transformer = CameraBasedTransformer(Vector2(800, 600), Vector2(50, 30), 1.5)
        world_positions = [
            Vector2(0, 0),
            Vector2(100, 50),
            Vector2(-50, -30),
        ...
        # When - 일괄 변환
        screen_positions = transformer.transform_multiple_points(world_positions)
        ...
        # Then - 결과 일치 확인
        assert len(screen_positions) == len(world_positions), "변환된 좌표 개수가 일치해야 함"
        
        for batch_pos, individual_pos in zip(screen_positions, individual_positions):
            assert batch_pos.distance_to(individual_pos) < 0.001, "일괄 변환과 개별 변환 결과가 일치해야 함"
    ...
    def test_가시_월드_영역_경계_계산_정확성_검증_성공_시나리오(self) -> None:
        ...
        assert abs(actual_width - expected_width) < 0.001, "가시 영역 너비가 정확해야 함"
        assert abs(actual_height - expected_height) < 0.001, "가시 영역 높이가 정확해야 함"
        ...
        assert abs(center_x - expected_center.x) < 0.001, "가시 영역 중심 X가 카메라 오프셋과 일치해야 함"
        assert abs(center_y - expected_center.y) < 0.001, "가시 영역 중심 Y가 카메라 오프셋과 일치해야 함"
    ...
    def test_줌_레벨_범위_제한_검증_성공_시나리오(self) -> None:
        ...
        assert transformer.zoom_level == 0.1, "줌 레벨 최소값이 0.1로 제한되어야 함"
        
        # When & Then - 최대값 제한 테스트
        transformer.zoom_level = 15.0  # 10.0 초과
        assert transformer.zoom_level == 10.0, "줌 레벨 최대값이 10.0으로 제한되어야 함"
        
        # When & Then - 정상 범위 테스트
        transformer.zoom_level = 2.5
        assert transformer.zoom_level == 2.5, "정상 범위의 줌 레벨은 그대로 설정되어야 함"