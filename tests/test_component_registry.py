...
class TestComponentRegistry:
    ...
    def test_컴포넌트_추가_성공_시나리오(
        ...
        # Then - 컴포넌트가 올바르게 추가됨
        assert registry.has_component(entity, HealthComponent), (
            '엔티티에 컴포넌트가 추가되어야 함'
        )
        retrieved = registry.get_component(entity, HealthComponent)
        assert retrieved is health_component, (
            '추가한 컴포넌트와 동일한 인스턴스가 반환되어야 함'
        )
    ...
    def test_컴포넌트_제거_성공_시나리오(
        ...
        assert registry.get_component(entity, HealthComponent) is None, (
            '컴포넌트 조회 시 None 반환되어야 함'
        )
    ...
    def test_컴포넌트_조회_기능_검증(
        ...
        # Then - 올바른 컴포넌트 조회
        assert (
            registry.get_component(entity, HealthComponent) is health_component
        )
        assert (
            registry.get_component(entity, PositionComponent)
            is position_component
        )
        ...
        assert registry.has_component(entity, PositionComponent), (
            '위치 컴포넌트 존재해야 함'
        )
        assert not registry.has_component(entity, VelocityComponent), (
            '속도 컴포넌트는 존재하지 않아야 함'
        )
    ...
    def test_타입별_엔티티_조회_기능(
        ...
        health_entities = list(
            registry.get_entities_with_component(HealthComponent)
        )
        ...
        position_entities = list(
            registry.get_entities_with_component(PositionComponent)
        )
        assert len(position_entities) == 1, (
            '위치 컴포넌트를 가진 엔티티는 1개여야 함'
        )
    ...
    def test_다중_컴포넌트_타입_엔티티_조회_기능(
        ...
        registry.add_component(
            entity3, HealthComponent(max_health=50, current_health=25)
        )

        # When - 헬스와 위치 컴포넌트를 모두 가진 엔티티 조회
        entities_with_both = list(
            registry.get_entities_with_components(
        ...
        assert len(entities_with_both) == 2, (
            '헬스와 위치 컴포넌트를 모두 가진 엔티티는 2개여야 함'
        )
    ...
    def test_레지스트리_통계_및_상태_정보_기능(
        ...
        # Then - 정확한 통계 정보 반환
        assert registry.get_component_count(HealthComponent) == 2, (
            '헬스 컴포넌트는 2개여야 함'
        )
        assert registry.get_component_count(PositionComponent) == 1, (
            '위치 컴포넌트는 1개여야 함'
        )
        assert registry.get_component_count(VelocityComponent) == 0, (
            '속도 컴포넌트는 0개여야 함'
        )

        assert registry.get_entity_component_count(entity1) == 2, (
            'entity1은 2개 컴포넌트를 가져야 함'
        )
        assert registry.get_entity_component_count(entity2) == 1, (
            'entity2는 1개 컴포넌트를 가져야 함'
        )

        assert len(registry) == 3, '전체 컴포넌트 개수는 3개여야 함'
        assert len(registry.get_all_component_types()) == 2, (
            '컴포넌트 타입은 2개여야 함'
        )
    ...
    def test_비활성_엔티티_조회_제외_기능(
        ...
        assert len(multi_entities) == 1, (
            '활성 엔티티만 다중 컴포넌트 조회되어야 함'
        )
        assert multi_entities[0][0] == active_entity

    def test_레지스트리_클리어_기능(self, registry: ComponentRegistry) -> None:
        """15. 레지스트리 클리어 기능
        ...
        registry.add_component(
            entity1, HealthComponent(max_health=100, current_health=80)
        )
