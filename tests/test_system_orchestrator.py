...
from src.core.system import ISystem, System
...
class TestSystemOrchestrator:
    ...
    def test_시스템_등록_성공_시나리오(
        ...
        # Then - 시스템이 올바르게 등록됨
        assert orchestrator.has_system("MovementSystem"), "시스템이 등록되어야 함"
    ...
    def test_시스템_등록_해제_성공_시나리오(
        ...
        # Then - 시스템이 올바르게 제거됨
        assert removed_system is movement_system, "제거된 시스템이 반환되어야 함"
        assert not orchestrator.has_system("MovementSystem"), "시스템이 제거되어야 함"
    ...
    def test_존재하지_않는_시스템_등록_해제_시나리오(
        ...
        # Then - None 반환
        assert removed_system is None, "존재하지 않는 시스템 제거 시 None 반환되어야 함"
    ...
    def test_시스템_실행_순서_우선순위_검증(
        ...
        # Then - 모든 시스템이 실행됨
        assert movement_system.update_count == 1, "Movement 시스템이 실행되어야 함"
        assert physics_system.update_count == 1, "Physics 시스템이 실행되어야 함"
        ...
        assert all(s['call_count'] == 1 for s in stats.values()), "모든 시스템이 한 번씩 호출되어야 함"
    ...
    def test_비활성_시스템_실행_제외_검증(
        ...
        assert movement_system.update_count == 0, "비활성 시스템은 업데이트되지 않아야 함"
        stats = orchestrator.get_execution_stats()
        assert stats["MovementSystem"]["call_count"] == 0, "비활성 시스템 호출 횟수는 0이어야 함"
    ...
    def test_시스템_우선순위_변경_기능(
        ...
        # When - 우선순위 변경
        new_priority = 999
        result = orchestrator.set_system_priority("MovementSystem", new_priority)
        
        # Then - 우선순위가 변경됨
        assert result is True, "우선순위 변경이 성공해야 함"
        assert movement_system.priority == new_priority, "시스템 우선순위가 변경되어야 함"
        
        # 존재하지 않는 시스템 우선순위 변경 시도
        result = orchestrator.set_system_priority("NonExistent", 100)
        assert result is False, "존재하지 않는 시스템 우선순위 변경은 실패해야 함"
    ...
    def test_시스템_그룹_관리_기능(
        ...
        # When - 시스템 그룹 생성
        result = orchestrator.create_system_group("GameplayGroup", ["MovementSystem", "PhysicsSystem"])
        ...
        assert not movement_system.enabled, "Movement 시스템이 비활성화되어야 함"
        ...
        result = orchestrator.create_system_group("InvalidGroup", ["NonExistent"])
        assert result is False, "존재하지 않는 시스템이 포함된 그룹 생성은 실패해야 함"
    ...
    def test_실행_통계_수집_기능(
        ...
        assert system_stats["max_time"] >= system_stats["avg_time"], "최대 실행 시간이 유효해야 함"
    ...
    def test_커스텀_정리_로직_실행_검증(
        ...
        # Then - 커스텀 정리 로직이 실행됨
        assert cleanup_system.cleaned_up, "커스텀 cleanup 메서드가 호출되어야 함"
    ...
    def test_모든_시스템_일괄_정리_기능(
        ...
        orchestrator.register_system(movement_system, "MovementSystem")
        orchestrator.register_system(physics_system, "PhysicsSystem")
        orchestrator.create_system_group("TestGroup", ["MovementSystem", "PhysicsSystem"])
        ...
        assert len(orchestrator.get_system_names()) == 0, "시스템 이름 목록이 비어야 함"
        assert len(orchestrator.get_execution_stats()) == 0, "실행 통계가 비어야 함"
    ...
    def test_시스템_조회_및_상태_확인_기능(
        ...
        assert "MovementSystem" in system_names and "PhysicsSystem" in system_names
