...
class TestEntityRenderSystem:
    ...
    def test_좌표_변환_시스템_통합_및_world_to_screen_적용_성공_시나리오(self) -> None:
        ...
        assert stats['culled_entities'] == 0, "화면 내 엔티티는 컬링되지 않아야 함"
    ...
    def test_화면_밖_컬링_최적화_시스템_동작_성공_시나리오(self) -> None:
        ...
        # Then - 컬링 통계 확인
        stats = self.render_system.get_render_stats()
        assert stats['visible_entities'] == 0, "화면 밖 엔티티는 렌더링되지 않아야 함"
    ...
    def test_플레이어_중앙_고정_렌더링_시스템_동작_성공_시나리오(self) -> None:
        ...
        # Then - 플레이어 렌더링 통계 확인
        stats = self.render_system.get_render_stats()
        assert stats['player_entities'] == 1, "플레이어 엔티티가 렌더링되어야 함"
        assert stats['visible_entities'] == 1, "플레이어가 가시적으로 렌더링되어야 함"
    ...
    def test_깊이_정렬_시스템_Y좌표_기준_동작_성공_시나리오(self) -> None:
        ...
        self.entity_manager.add_component(bg_entity, PositionComponent(x=0, y=100))
        self.entity_manager.add_component(bg_entity, RenderComponent(
        ...
        self.entity_manager.add_component(front_entity, PositionComponent(x=0, y=50))
        self.entity_manager.add_component(front_entity, RenderComponent(
            layer=RenderLayer.ENTITIES, color=(255, 0, 0)
        ))
        entities.append(front_entity)
        
        # 엔티티 레이어, Y=200 (뒤쪽)
        back_entity = self.entity_manager.create_entity()
        self.entity_manager.add_component(back_entity, PositionComponent(x=0, y=200))
        self.entity_manager.add_component(back_entity, RenderComponent(
            layer=RenderLayer.ENTITIES, color=(0, 255, 0)
        ))
        ...
        assert sorted_entities[0] == bg_entity, "배경이 맨 뒤에 렌더링되어야 함"
        assert sorted_entities[1] == front_entity, "Y좌표가 작은 엔티티가 먼저 렌더링되어야 함"
        assert sorted_entities[2] == back_entity, "Y좌표가 큰 엔티티가 나중에 렌더링되어야 함"
    ...
    def test_회전_처리_시스템_동작_및_캐싱_성공_시나리오(self) -> None:
        ...
        # Then - 회전 처리 및 캐싱 확인
        assert first_stats['rotated_sprites'] >= 1, "회전된 스프라이트가 생성되어야 함"
        assert second_stats['visible_entities'] == 1, "회전된 엔티티가 렌더링되어야 함"
    ...
    def test_렌더링_성능_통계_수집_정확성_검증_성공_시나리오(self) -> None:
        ...
        self.entity_manager.add_component(normal_entity, PositionComponent(0, 0))
        ...
        self.entity_manager.add_component(player_entity, PositionComponent(0, 0))
        self.entity_manager.add_component(player_entity, RenderComponent())
        
        # 화면 밖 엔티티 (컬링 대상)
        culled_entity = self.entity_manager.create_entity()
        self.entity_manager.add_component(culled_entity, PositionComponent(-1000, -1000))
        ...
        assert stats['player_entities'] == 1, "플레이어 엔티티 수가 정확해야 함"
    ...
    def test_화면_크기_변경_시_적응_처리_성공_시나리오(self) -> None:
        ...
        assert initial_center == (400, 300), "초기 화면 중앙이 (400, 300)이어야 함"
        ...
        assert self.render_system._screen_width == 1024, "화면 너비가 업데이트되어야 함"
        assert self.render_system._screen_height == 768, "화면 높이가 업데이트되어야 함"
        assert self.render_system._screen_center == (512, 384), "화면 중앙이 업데이트되어야 함"
    ...
    def test_컬링_마진_설정_변경_동작_성공_시나리오(self) -> None:
        ...
        """
        # Given - 초기 컬링 마진 확인
        assert self.render_system._cull_margin == 50, "초기 컬링 마진이 50이어야 함"

        # When - 컬링 마진 변경
        self.render_system.set_cull_margin(100)

        # Then - 컬링 마진이 변경되었는지 확인
        assert self.render_system._cull_margin == 100, "컬링 마진이 100으로 변경되어야 함"
        
        # 음수 값 설정 시 0으로 제한되는지 확인
        self.render_system.set_cull_margin(-10)
        assert self.render_system._cull_margin == 0, "음수 마진은 0으로 제한되어야 함"
    ...
    def test_회전_캐시_메모리_관리_및_정리_성공_시나리오(self) -> None:
        ...
        # Then - 캐시가 비워졌는지 확인
        assert len(self.render_system._rotation_cache) == 0, "캐시가 정리되어야 함"
    ...
    def test_시스템_초기화_및_cleanup_처리_성공_시나리오(self) -> None:
        ...
        # Then - 정리 후 상태 확인
        assert len(new_system._rotation_cache) == 0, "정리 후 회전 캐시가 비워져야 함"