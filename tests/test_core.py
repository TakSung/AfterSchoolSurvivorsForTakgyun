...
class TestEntity:
    ...
    def test_엔티티_생명주기_관리_성공(self, entity_instance: Entity) -> None:
        ...
        assert entity_instance.active is False, (
            '비활성화 후 비활성 상태여야 함'
        )
    ...
    def test_엔티티_동등성_검사_ID_기반_성공(self) -> None:
        ...
        assert entity1 == entity3, (
            f'같은 ID를 가진 엔티티는 동등해야 함: {entity1.entity_id}'
        )
        assert entity1 != entity2, (
            '다른 ID를 가진 엔티티는 다르다고 판단되어야 함'
        )
        assert entity1 != 'not_an_entity', (
            '엔티티가 아닌 객체와는 다르다고 판단되어야 함'
        )
    ...
    def test_엔티티_해시_가능성_컬렉션_사용_성공(self) -> None:
        ...
        assert len(entity_set) == 2, (
            f'set에서 중복 엔티티가 제거되어야 함: {len(entity_set)}'
        )
...
class TestComponent:
    ...
    def test_컴포넌트_생성_dataclass_필드_성공(
        self, health_component: MockHealthComponent
    ) -> None:
        ...
        assert health_component.current_hp == expected_hp, (
            f'현재 HP가 올바르게 설정되어야 함: {health_component.current_hp}'
        )
        assert health_component.max_hp == expected_max_hp, (
            f'최대 HP가 올바르게 설정되어야 함: {health_component.max_hp}'
        )

    def test_컴포넌트_유효성_검증_성공(
        self, health_component: MockHealthComponent
    ) -> None:
        ...
        assert 0 <= health_component.current_hp <= health_component.max_hp, (
            '테스트 전제 조건 확인'
        )
        ...
        assert is_valid is True, (
            f'유효한 HP 값에 대해 True를 반환해야 함: current={health_component.current_hp}, max={health_component.max_hp}'
        )
    ...
    def test_컴포넌트_유효성_검증_범위_초과_실패(self) -> None:
        ...
        for invalid_component in invalid_cases:
            ...
            assert is_valid is False, (
                f'유효하지 않은 HP 값에 대해 False를 반환해야 함: current={invalid_component.current_hp}, max={invalid_component.max_hp}'
            )

    def test_컴포넌트_복사_독립성_성공(
        self, health_component: MockHealthComponent
    ) -> None:
        ...
        assert copied_component is not health_component, (
            '복사본은 원본과 다른 객체여야 함'
        )
        assert copied_component.current_hp == original_hp, (
            f'복사본의 HP가 원본과 같아야 함: {copied_component.current_hp} != {original_hp}'
        )
        assert copied_component.max_hp == original_max_hp, (
            f'복사본의 최대 HP가 원본과 같아야 함: {copied_component.max_hp} != {original_max_hp}'
        )
        ...
        # Then - 원본은 영향받지 않아야 함
        assert health_component.current_hp == original_hp, (
            '원본 컴포넌트는 복사본 수정에 영향받지 않아야 함'
        )

    def test_컴포넌트_직렬화_역직렬화_성공(
        self, position_component: MockPositionComponent
    ) -> None:
        ...
        assert isinstance(serialized_data, dict), (
            '직렬화된 데이터는 dict 타입이어야 함'
        )
        assert 'x' in serialized_data, (
            '직렬화된 데이터에 x 필드가 포함되어야 함'
        )
        assert 'y' in serialized_data, (
            '직렬화된 데이터에 y 필드가 포함되어야 함'
        )

        assert restored_component.x == approx(original_x), (
            f'X 좌표가 정확히 복원되어야 함: {restored_component.x} != {original_x}'
        )
        assert restored_component.y == approx(original_y), (
            f'Y 좌표가 정확히 복원되어야 함: {restored_component.y} != {original_y}'
        )

        # 타입 검증
        assert isinstance(restored_component, MockPositionComponent), (
            '복원된 객체는 올바른 타입이어야 함'
        )
...
class TestSystem:
    ...
    def test_시스템_생성_기본값_설정_성공(
        self, movement_system: MockMovementSystem
    ) -> None:
        ...
        assert movement_system.priority == 1, (
            f'기본 우선순위는 1이어야 함: {movement_system.priority}'
        )
        assert movement_system.enabled is True, (
            f'기본적으로 활성화되어야 함: {movement_system.enabled}'
        )
        assert movement_system.initialized is False, (
            f'초기화되지 않은 상태여야 함: {movement_system.initialized}'
        )
        assert movement_system.update_call_count == 0, (
            '업데이트 호출 횟수는 0이어야 함'
        )

    @pytest.mark.parametrize(
        'priority,enabled,expected_order',
        [
            (0, True, '최우선'),
            (5, True, '보통'),
            (10, True, '후순위'),
    ...
    def test_시스템_상태_관리_다양한_시나리오(
        ...
        assert actual_priority == priority, (
            f'우선순위가 올바르게 설정되어야 함: {actual_priority} != {priority}'
        )
        assert actual_enabled == enabled, (
            f'활성화 상태가 올바르게 설정되어야 함: {actual_enabled} != {enabled}'
        )
        ...
        if expected_order == '최우선':
            ...
            assert not enabled, (
                '비활성화 시나리오에서는 enabled가 False여야 함'
            )

    def test_시스템_초기화_상태_변경_성공(
        self, movement_system: MockMovementSystem
    ) -> None:
        ...
        assert movement_system.initialized is True, (
            'initialize() 호출 후 초기화된 상태여야 함'
        )

    def test_시스템_필수_컴포넌트_목록_반환_성공(
        self, movement_system: MockMovementSystem
    ) -> None:
        ...
        assert MockHealthComponent in required_components, (
            f'MockHealthComponent가 필수 컴포넌트에 포함되어야 함: {required_components}'
        )
        assert len(required_components) > 0, (
            '최소 하나의 필수 컴포넌트가 있어야 함'
        )

    def test_시스템_업데이트_호출_횟수_추적_성공(
        self, movement_system: MockMovementSystem
    ) -> None:
        ...
        assert movement_system.update_call_count == 1, (
            f'업데이트가 한 번 호출되어야 함: {movement_system.update_call_count}'
        )
        assert movement_system.processed_entities == test_entity_manager, (
            '전달된 엔티티 매니저가 처리되어야 함'
        )
        ...
        # Then - 호출 횟수가 증가해야 함
        assert movement_system.update_call_count == 2, (
            f'두 번째 업데이트 후 호출 횟수는 2여야 함: {movement_system.update_call_count}'
        )
...
class TestECSIntegration:
    ...
    def ecs_setup(
        self,
    ) -> tuple[Entity, MockHealthComponent, MockMovementSystem]:
        """ECS 통합 테스트용 설정 픽스처"""
    ...
    def test_엔티티_컴포넌트_시스템_통합_동작_성공(
        self, ecs_setup: tuple[Entity, MockHealthComponent, MockMovementSystem]
    ) -> None:
        ...
        assert system.update_call_count == 1, (
            '시스템 업데이트가 한 번 호출되어야 함'
        )
        assert entity.active is True, '엔티티가 활성 상태를 유지해야 함'
        assert component.validate() is True, '컴포넌트 데이터가 유효해야 함'

        # 시스템이 엔티티를 처리했는지 확인
        assert len(system.processed_entities) == 1, (
            '시스템이 하나의 엔티티를 처리해야 함'
        )
        assert system.processed_entities[0] == entity, (
            '시스템이 올바른 엔티티를 처리해야 함'
        )
