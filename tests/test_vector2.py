...
class TestVector2:
    ...
    def test_벡터_크기_계산_정확성_검증_성공_시나리오(self) -> None:
        ...
        assert math.isclose(vector.magnitude, expected_magnitude), (
            '벡터 크기가 정확해야 함'
        )
        assert math.isclose(vector.magnitude_squared, 25.0), (
            '벡터 크기 제곱이 정확해야 함'
        )
        ...
        assert zero_vector.magnitude_squared == 0.0, (
            '영벡터의 크기 제곱은 0이어야 함'
        )
    ...
    def test_벡터_거리_계산_정확성_검증_성공_시나리오(self) -> None:
        ...
        assert math.isclose(distance_squared, 25.0), (
            '두 점 간 거리 제곱이 정확해야 함'
        )
        ...
        assert same_point1.distance_to(same_point2) == 0.0, (
            '동일한 점들 간 거리는 0이어야 함'
        )
    ...
    def test_벡터_내적_외적_계산_정확성_검증_성공_시나리오(self) -> None:
        ...
        assert math.isclose(dot_product, expected_dot), (
            '내적 계산이 정확해야 함'
        )
        ...
        assert math.isclose(cross_product, expected_cross), (
            '외적 계산이 정확해야 함'
        )
    ...
    def test_벡터_클래스_메서드_정확성_검증_성공_시나리오(self) -> None:
        ...
        assert math.isclose(angle_vector.x, 0.0, abs_tol=1e-10), (
            '90도 벡터 x가 0이어야 함'
        )
