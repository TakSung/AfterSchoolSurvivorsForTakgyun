...
class TestMapComponent:
    ...
    def test_맵_컴포넌트_기본_초기화_검증_성공_시나리오(self) -> None:
        ...
        # Then - 기본값 확인
        assert map_comp.tile_size == 50, "기본 타일 크기는 50이어야 함"
        assert map_comp.world_width == 2000.0, "기본 월드 너비는 2000.0이어야 함"
        assert map_comp.world_height == 2000.0, "기본 월드 높이는 2000.0이어야 함"
        assert map_comp.grid_color == (100, 100, 100), "기본 격자 색상 확인"
        assert map_comp.background_color == (50, 50, 60), "기본 배경 색상 확인"
    ...
    def test_맵_컴포넌트_커스텀_초기화_검증_성공_시나리오(self) -> None:
        ...
        # Then - 커스텀 값 확인
        assert map_comp.tile_size == 64, "커스텀 타일 크기 확인"
        assert map_comp.world_width == 1024.0, "커스텀 월드 너비 확인"
        assert map_comp.world_height == 768.0, "커스텀 월드 높이 확인"
        assert map_comp.grid_color == (200, 200, 200), "커스텀 격자 색상 확인"
        assert map_comp.background_color == (10, 10, 10), "커스텀 배경 색상 확인"
    ...
    def test_타일_위치_계산_기능_검증_성공_시나리오(self) -> None:
        ...
        """
        # Given - 기본 맵 컴포넌트 (타일 크기 50)
        map_comp = MapComponent()

        # When & Then - 다양한 월드 좌표에 대한 타일 좌표 계산
        assert map_comp.get_tile_at_position(0.0, 0.0) == (0, 0)
        assert map_comp.get_tile_at_position(25.0, 25.0) == (0, 0)
        assert map_comp.get_tile_at_position(50.0, 50.0) == (1, 1)
        assert map_comp.get_tile_at_position(125.0, 75.0) == (2, 1)
        assert map_comp.get_tile_at_position(-25.0, -25.0) == (-1, -1)
        assert map_comp.get_tile_at_position(-50.0, -50.0) == (-1, -1)
    ...
    def test_타일_월드_위치_계산_기능_검증_성공_시나리오(self) -> None:
        ...
        assert map_comp.get_tile_world_position(1, 1) == (50.0, 50.0)
        assert map_comp.get_tile_world_position(2, 3) == (100.0, 150.0)
        assert map_comp.get_tile_world_position(-1, -1) == (-50.0, -50.0)
        assert map_comp.get_tile_world_position(-2, -3) == (-100.0, -150.0)
    ...
    def test_가시_타일_범위_계산_기능_검증_성공_시나리오(self) -> None:
        ...
        assert max_tile_x == 17, "최대 X 타일은 17이어야 함 (800/50 + 1)"
        assert max_tile_y == 13, "최대 Y 타일은 13이어야 함 (600/50 + 1)"
    ...
    def test_가시_타일_범위_카메라_오프셋_적용_검증_성공_시나리오(self) -> None:
        ...
        # Then - 오프셋이 반영된 타일 범위 확인
        min_tile_x, min_tile_y = min_tile
        max_tile_x, max_tile_y = max_tile
        
        # 오프셋 -100은 타일 2개만큼 이동 (100/50 = 2)
        assert min_tile_x == 1, "오프셋 적용된 최소 X 타일 확인"
        assert min_tile_y == 1, "오프셋 적용된 최소 Y 타일 확인"
        assert max_tile_x == 19, "오프셋 적용된 최대 X 타일 확인"
        assert max_tile_y == 15, "오프셋 적용된 최대 Y 타일 확인"
    ...
    def test_맵_컴포넌트_유효성_검사_실패_시나리오들(self) -> None:
        ...
        invalid_comp = MapComponent(background_color=(-1, 50, 50))  # -1 < 0
        assert invalid_comp.validate() is False
