...
class TestEnemySpawnerSystem:
    ...
    def setup_method(self) -> None:
        ...
        # 기본 좌표 변환 설정
        self.mock_coordinate_manager.screen_to_world.return_value = Vector2(100, 100)
        
        with patch('src.systems.enemy_spawner_system.CoordinateManager.get_instance', 
    ...
    def test_시스템_초기화_및_기본_설정_검증_성공_시나리오(self) -> None:
        ...
        # Then - 기본 설정값 확인
        assert spawn_info['base_spawn_interval'] == 2.0, "기본 스폰 간격이 2초여야 함"
        assert spawn_info['max_enemies'] == 20, "최대 적 수가 20이어야 함"
        assert spawn_info['game_time'] == 0.0, "게임 시간이 0으로 초기화되어야 함"
        assert spawn_info['spawn_timer'] == 0.0, "스폰 타이머가 0으로 초기화되어야 함"
        assert spawn_info['current_spawn_interval'] == 2.0, "현재 스폰 간격이 기본값과 같아야 함"
    ...
    def test_최대_적_수_제한_동작_검증_성공_시나리오(self) -> None:
        ...
        for i in range(4):
            ...
            self.entity_manager.add_component(enemy, PositionComponent(x=i*10, y=0))
            enemies.append(enemy)
        
        # Debug - 적 개수 확인
        entities_with_components = self.entity_manager.get_entities_with_components(
        ...
        is_within_limit_before = self.spawner_system._is_enemy_count_within_limit(
            self.entity_manager
        )
        ...
        is_within_limit_after = self.spawner_system._is_enemy_count_within_limit(
            self.entity_manager
        )
        
        # Then - 정확한 제한 동작 확인
        assert current_count_before == 4, f"4개 적이 정확히 카운팅되어야 함, 실제: {current_count_before}"
        assert is_within_limit_before is True, "4개일 때는 스폰 가능해야 함"
        assert current_count_after == 5, f"5개 적이 정확히 카운팅되어야 함, 실제: {current_count_after}"
    ...
    def test_전체_스폰_조건_통합_검증_성공_시나리오(self) -> None:
        ...
        # Given - 시간은 충분, 적 개수는 제한 초과
        self.spawner_system._current_spawn_timer = 3.0
        for _ in range(10):  # 최대 개수만큼 생성
            enemy = self.entity_manager.create_entity()
            self.entity_manager.add_component(enemy, EnemyComponent())
            self.entity_manager.add_component(enemy, PositionComponent(x=0, y=0))
        
        should_spawn_count_exceeded = self.spawner_system._should_spawn_enemy(
            self.entity_manager
        )
        
        # Given - 시간과 개수 모두 조건 만족
        # 적 하나 제거하여 개수 조건 만족
        enemies = self.entity_manager.get_entities_with_components(EnemyComponent)
        if enemies:
            self.entity_manager.destroy_entity(enemies[0])
        
        should_spawn_all_conditions_met = self.spawner_system._should_spawn_enemy(
            self.entity_manager
        )
        
        # Then - 복합 조건 검증
        assert should_spawn_time_not_ready is False, "시간 부족 시 스폰하지 않아야 함"
        assert should_spawn_count_exceeded is False, "개수 초과 시 스폰하지 않아야 함"
        assert should_spawn_all_conditions_met is True, "모든 조건 만족 시 스폰해야 함"
    ...
    def test_난이도_기반_스폰_간격_조정_검증_성공_시나리오(self) -> None:
        ...
        assert interval_max_difficulty >= 0.5, "최소 간격 0.5초 이상 유지되어야 함"
        
        # 정확한 계산 검증 (30초: 5% 감소)
        expected_30s = 2.0 * 0.95  # 1.9초
        assert abs(interval_30s - expected_30s) < 0.01, f"30초 후 간격이 {expected_30s}초 근처여야 함"
    ...
    def test_화면_가장자리_스폰_위치_계산_검증_성공_시나리오(
        ...
        self.mock_coordinate_manager.screen_to_world.side_effect = screen_to_world_side_effect
        ...
        # 좌표 변환이 호출되었는지 확인
        assert self.mock_coordinate_manager.screen_to_world.called, "좌표 변환이 호출되어야 함"
    ...
    def test_적_엔티티_생성_및_컴포넌트_구성_검증_성공_시나리오(self) -> None:
        ...
        """
        # Given - 스폰 조건을 만족하는 상황 설정
        self.spawner_system._current_spawn_timer = 3.0
        
        # Given - 스폰 위치 계산이 성공하도록 설정
        with patch.object(self.spawner_system, '_calculate_spawn_position', 
                         return_value=(100.0, 200.0)):
            # When - 적 스폰 실행
            initial_enemy_count = self.spawner_system._get_current_enemy_count(
                self.entity_manager
            )
            self.spawner_system._spawn_enemy(self.entity_manager)
            final_enemy_count = self.spawner_system._get_current_enemy_count(
                self.entity_manager
            )
        
        # Then - 적 엔티티 생성 확인
        assert final_enemy_count == initial_enemy_count + 1, "적이 1개 생성되어야 함"
        ...
        # 필수 컴포넌트 존재 확인
        assert self.entity_manager.has_component(enemy, EnemyComponent), "EnemyComponent 필요"
        assert self.entity_manager.has_component(enemy, PositionComponent), "PositionComponent 필요"
        assert self.entity_manager.has_component(enemy, EnemyAIComponent), "EnemyAIComponent 필요"
        
        # AI 컴포넌트 설정 검증
        ai_component = self.entity_manager.get_component(enemy, EnemyAIComponent)
        assert ai_component is not None, "AI 컴포넌트가 존재해야 함"
        assert isinstance(ai_component.ai_type, AIType), "AI 타입이 올바르게 설정되어야 함"
    ...
    def test_시스템_업데이트_전체_플로우_검증_성공_시나리오(self) -> None:
        ...
        테스트할 범위: update() 메서드와 관련 서브루틴들
        커버하는 함수 및 데이터: 전체 시스템 동작 플로우
        기대되는 안정성: 매 프레임 안정적인 스폰 시스템 동작
        """
        # Given - 스폰 조건을 만족하도록 설정
        self.spawner_system.set_max_enemies(5)
        ...
        with patch.object(self.spawner_system, '_calculate_spawn_position', 
            ...
            after_first_update_enemies = self.spawner_system._get_current_enemy_count(
                self.entity_manager
            )
            
            # When - 두 번째 업데이트 (시간 충족)
            self.spawner_system.update(self.entity_manager, 2.0)
            
            after_second_update_timer = self.spawner_system._current_spawn_timer
            after_second_update_enemies = self.spawner_system._get_current_enemy_count(
                self.entity_manager
            )
        ...
        # 게임 시간 누적 확인
        assert self.spawner_system._game_time > initial_game_time, "게임 시간이 증가해야 함"
        
        # 첫 번째 업데이트 후
        assert after_first_update_timer == 0.5, "타이머가 0.5초 증가해야 함"
        assert after_first_update_enemies == 0, "시간 부족으로 스폰되지 않아야 함"
        
        # 두 번째 업데이트 후
        assert after_second_update_timer == 0.0, "스폰 후 타이머가 리셋되어야 함"
        assert after_second_update_enemies == 1, "조건 만족으로 적이 스폰되어야 함"
    ...
    def test_설정_변경_메서드_동작_검증_성공_시나리오(self) -> None:
        ...
        # Then - 설정 변경 확인
        assert initial_spawn_info['base_spawn_interval'] == 2.0, "초기 간격이 2초여야 함"
        assert initial_spawn_info['max_enemies'] == 20, "초기 최대 적 수가 20이어야 함"
        
        assert updated_spawn_info['base_spawn_interval'] == 1.5, "간격이 1.5초로 변경되어야 함"
        assert updated_spawn_info['max_enemies'] == 15, "최대 적 수가 15로 변경되어야 함"
        ...
        # Then - 잘못된 값은 무시되어야 함
        assert final_spawn_info['base_spawn_interval'] == 1.5, "잘못된 간격 값은 무시되어야 함"
        assert final_spawn_info['max_enemies'] == 15, "잘못된 최대 적 수는 무시되어야 함"
    ...
    def test_좌표_관리자_없을_때_예외_처리_검증_성공_시나리오(self) -> None:
        ...
        initial_count = self.spawner_system._get_current_enemy_count(self.entity_manager)
        self.spawner_system._spawn_enemy(self.entity_manager)
        final_count = self.spawner_system._get_current_enemy_count(self.entity_manager)
        
        # Then - 적이 생성되지 않아야 함
        assert final_count == initial_count, "좌표 계산 실패 시 적이 생성되지 않아야 함"