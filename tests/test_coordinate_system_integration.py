...
from src.core.coordinate_transformer import CoordinateSpace, ICoordinateTransformer
...
class TestCoordinateSystemIntegration:
    def test_전체_좌표_변환_시스템_정확성_검증_성공_시나리오(self) -> None:
        ...
        basic_transformer = CameraBasedTransformer(screen_size, camera_offset, zoom_level)
        cached_transformer = CachedCameraTransformer(screen_size, camera_offset, zoom_level)
        ...
        for world_pos in test_positions:
            ...
            assert basic_screen.distance_to(cached_screen) < 0.001, f"변환기별 결과가 일치해야 함: {world_pos}"
            
            # 왕복 변환 정확성 검증
            basic_world = basic_transformer.screen_to_world(basic_screen)
            cached_world = cached_transformer.screen_to_world(cached_screen)
            
            assert world_pos.distance_to(basic_world) < 0.001, f"기본 변환기 왕복 변환 정확성: {world_pos}"
            assert world_pos.distance_to(cached_world) < 0.001, f"캐시 변환기 왕복 변환 정확성: {world_pos}"
    ...
    def test_극한_상황_좌표_변환_안정성_검증_성공_시나리오(self) -> None:
        ...
        extreme_test_cases = [
            ...
            (Vector2(10000, 10000), Vector2(5000, 5000), 0.1, "매우 큰 좌표 + 최소 줌"),
            (Vector2(-10000, -10000), Vector2(-5000, -5000), 10.0, "음수 좌표 + 최대 줌"),
            (Vector2(0.001, 0.001), Vector2(0, 0), 5.0, "매우 작은 좌표 + 고배율 줌"),
            (Vector2(1000000, 1000000), Vector2(999999, 999999), 1.0, "거대 좌표값"),
        ]
        
        # When & Then - 극한 상황 테스트
        for world_pos, camera_offset, zoom_level, description in extreme_test_cases:
            ...
            try:
                ...
                # 수치 안정성 확인 (상대적 오차 허용)
                relative_error = world_pos.distance_to(world_back) / max(world_pos.magnitude, 1.0)
                assert relative_error < 0.01, f"극한 상황 정확성 실패: {description}"
                
                # NaN, Infinity 체크
                assert not (screen_pos.x != screen_pos.x or screen_pos.y != screen_pos.y), f"NaN 발생: {description}"
                assert abs(screen_pos.x) < float('inf') and abs(screen_pos.y) < float('inf'), f"Infinity 발생: {description}"
                
            except (OverflowError, ZeroDivisionError) as e:
                pytest.fail(f"극한 상황 처리 실패 ({description}): {e}")

    def test_다중_스레드_환경_좌표_변환_안전성_검증_성공_시나리오(self) -> None:
        ...
        for thread_id, thread_results in results.items():
            for i, (pos, screen_pos, world_pos) in enumerate(thread_results):
                ...
                assert screen_pos.distance_to(ref_screen) < 0.001, f"스레드 {thread_id}: 화면 좌표 불일치"
                assert world_pos.distance_to(ref_world) < 0.001, f"스레드 {thread_id}: 월드 좌표 불일치"
    ...
    def test_캐시_성능_및_메모리_효율성_검증_성공_시나리오(self) -> None:
        ...
        small_cache_transformer = CachedCameraTransformer(Vector2(800, 600), cache_size=10)
        
        # 반복 패턴이 있는 좌표들과 고유 좌표들 혼합 생성
        base_positions = [Vector2(i * 20, i * 10) for i in range(8)]  # 반복될 좌표들
        unique_positions = [Vector2(i * 100, i * 50) for i in range(20)]  # 캐시 제거를 유발할 좌표들
        ...
        # Then - 캐시 효과 검증
        total_ops = cache_stats['total']['hits'] + cache_stats['total']['misses']
        hit_rate = cache_stats['total']['hit_rate']
        
        expected_total = len(base_positions) * 5 + len(unique_positions)
        assert total_ops == expected_total, f"총 연산 수가 일치해야 함: {total_ops} vs {expected_total}"
        assert hit_rate > 0.3, f"반복 패턴에서 캐시 히트율이 30% 이상이어야 함: {hit_rate:.2%}"
        
        # Then - 메모리 제한 확인
        current_cache_size = cache_stats['total']['current_size']
        max_cache_size = cache_stats['total']['max_size']
        
        assert current_cache_size <= max_cache_size, "캐시 크기가 제한을 초과하지 않아야 함"
        
        # 캐시 제거는 고유 좌표가 캐시 크기를 초과할 때 발생
        if len(unique_positions) > max_cache_size // 2:  # world_to_screen 캐시만 고려
            assert cache_stats['total']['evictions'] > 0, f"캐시 크기 제한으로 인한 제거가 발생해야 함: evictions={cache_stats['total']['evictions']}"
    ...
    def test_게임_시스템_통합_시나리오_검증_성공_시나리오(self) -> None:
        ...
        # 다수의 적 엔티티 위치
        enemy_positions = [
            Vector2(player_world_pos.x + random.uniform(-200, 200),
        ...
        for frame in range(frame_count):
            ...
            # 플레이어 이동 시뮬레이션
            player_world_pos += Vector2(random.uniform(-2, 2), random.uniform(-2, 2))
            ...
            if visible_enemies:
                ...
                # 공격 방향 계산 (월드 좌표)
                attack_direction = (closest_enemy[0] - player_world_pos).normalized()
            
            # 4. RenderSystem - 다중 좌표 변환 (배치 처리)
            all_world_positions = [player_world_pos] + [pos for pos, _ in visible_enemies]
            all_screen_positions = transformer.transform_multiple_points(all_world_positions)
        ...
        assert max_frame_time < 0.02, f"최대 프레임 시간이 20ms 미만이어야 함: {max_frame_time*1000:.1f}ms"
        
        # Then - 캐시 효과 확인
        cache_stats = transformer.get_coordinate_cache_stats()
        final_hit_rate = cache_stats['total']['hit_rate']
        
        assert final_hit_rate > 0.3, f"게임 루프에서 캐시 히트율이 30% 이상이어야 함: {final_hit_rate:.2%}"
    ...
    def test_좌표_변환_시스템_에러_복구_검증_실패_시나리오(self) -> None:
        ...
        for invalid_pos in invalid_inputs:
            try:
                ...
                if abs(result.x) == float('inf') or abs(result.y) == float('inf'):
        ...
        assert recovery_result.distance_to(normal_result) < 0.001, "오류 후 시스템이 정상 복구되어야 함"
    ...
    def test_대용량_데이터_처리_성능_검증_성공_시나리오(self) -> None:
        ...
        transformer = CachedCameraTransformer(Vector2(1920, 1080), cache_size=5000)
        ...
        individual_results = [transformer.world_to_screen(pos) for pos in large_dataset]
        individual_time = time.perf_counter() - individual_start
        
        # Then - 결과 정확성 확인
        assert len(batch_results) == len(large_dataset), "배치 처리 결과 개수가 일치해야 함"
        assert len(individual_results) == len(large_dataset), "개별 처리 결과 개수가 일치해야 함"
        
        for i, (batch_result, individual_result) in enumerate(zip(batch_results, individual_results)):
            assert batch_result.distance_to(individual_result) < 0.001, f"처리 방식별 결과 일치 실패: index {i}"
        
        # Then - 성능 요구사항 확인
        operations_per_sec_batch = len(large_dataset) / batch_time if batch_time > 0 else 0
        operations_per_sec_individual = len(large_dataset) / individual_time if individual_time > 0 else 0
        
        # 10,000개 좌표를 1초 내 처리 (성능 목표)
        assert operations_per_sec_batch >= 10000, f"배치 처리 성능 부족: {operations_per_sec_batch:.0f} ops/sec"
        
        # 배치 처리가 개별 처리보다 효율적이어야 함 (캐시 효과)
        cache_stats = transformer.get_coordinate_cache_stats()
        if cache_stats['total']['hits'] > 0:
            # 캐시 히트가 있는 경우 배치 처리가 더 빨라야 함
            assert batch_time <= individual_time, f"배치 처리가 더 효율적이어야 함: {batch_time:.3f}s vs {individual_time:.3f}s"