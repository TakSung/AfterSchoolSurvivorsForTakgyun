...
def main():
    ...
    while running:
        ...
        pygame.draw.circle(screen, (100, 100, 100), (center_x, center_y),
                          int(movement_comp.dead_zone_radius), 2)
...
def draw_grid(screen, camera_comp, grid_size):
    ...
    for x in range(-grid_size, screen_width + grid_size, grid_size):
        ...
        pygame.draw.line(screen, (50, 50, 50), (grid_x, 0), (grid_x, screen_height))

    for y in range(-grid_size, screen_height + grid_size, grid_size):
        grid_y = y + offset_y % grid_size
        pygame.draw.line(screen, (50, 50, 50), (0, grid_y), (screen_width, grid_y))
...
def display_info(screen, movement_comp, camera_comp, mouse_pos):
    ...
    text = font.render(f"Player World Pos: ({world_pos[0]:.1f}, {world_pos[1]:.1f})",
                      True, (255, 255, 255))
    ...
    text = font.render(f"Camera Offset: ({camera_offset[0]:.1f}, {camera_offset[1]:.1f})",
                      True, (255, 255, 255))
    screen.blit(text, (10, y_offset))
    y_offset += line_height
    ...
    # Player direction
    direction = movement_comp.direction
    text = font.render(f"Direction: ({direction[0]:.2f}, {direction[1]:.2f})",
                      True, (255, 255, 255))
    screen.blit(text, (10, y_offset))
